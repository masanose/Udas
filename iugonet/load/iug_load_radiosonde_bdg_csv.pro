...
;  of the troposphere in csv format taken by the radiosonde at Bandung 
;  and loads data into tplot format.
;
;SYNTAX:
; iug_load_radiosonde_bdg_csv, downloadonly=downloadonly, trange=trange, verbose=verbose
;
;KEYWOARDS:
...
pro iug_load_radiosonde_bdg_csv, downloadonly=downloadonly, $
  trange=trange, $
  verbose=verbose
...
if ~size(fns,/type) then begin       
  ...
  ;===============================
   ...
   source.local_data_dir =  root_data_dir() + 'iugonet/rish/misc/bdg/radiosonde/csv/'
   source.remote_data_dir = 'http://database.rish.kyoto-u.ac.jp/arch/iugonet/sonde/data/bandung/csv/'
...
if (downloadonly eq 0) then begin
  ...
  ;==============
   for j=0L,n_elements(local_paths)-1 do begin
      file= local_paths[j] 
      if file_test(/regular,file) then  dprint,'Loading Bandung sonde data file: ',file $
      else begin
         dprint,'Bandung sonde data file',file,'not found. Skipping'
         continue
      endelse
     ...
     ;---Convert time from UT to UNIX time
      ...
      for i=0, n_elements(ht)-1 do begin
         idx = where((height_data ge ht[i]-dh/2.0) and (height_data lt ht[i]+dh/2.0),cnt)
  ...
  ;---Acknowlegment string (use for creating tplot vars)
   ...
   if size(sonde_press,/type) eq 4 then begin 
     ;---Create tplot variables and options
      ...
      store_data,'iug_radiosonde_bdg_press',data={x:sonde_time, y:sonde_press, v:ht/1000.0},dlimit=dlimit
      options,'iug_radiosonde_bdg_press',ytitle='RSND-bdg!CHeight!C[km]',ztitle='Press.!C[hPa]'
      store_data,'iug_radiosonde_bdg_temp',data={x:sonde_time, y:sonde_temp, v:ht/1000.0},dlimit=dlimit
      options,'iug_radiosonde_bdg_temp',ytitle='RSND-bdg!CHeight!C[km]',ztitle='Temp.!C[deg.]'
      store_data,'iug_radiosonde_bdg_rh',data={x:sonde_time, y:sonde_rh, v:ht/1000.0},dlimit=dlimit
      options,'iug_radiosonde_bdg_rh',ytitle='RSND-bdg!CHeight!C[km]',ztitle='RH!C[%]'
      store_data,'iug_radiosonde_bdg_uwnd',data={x:sonde_time, y:sonde_uwind, v:ht/1000.0},dlimit=dlimit
      options,'iug_radiosonde_bdg_uwnd',ytitle='RSND-bdg!CHeight!C[km]',ztitle='uwnd!C[m/s]'
      store_data,'iug_radiosonde_bdg_vwnd',data={x:sonde_time, y:sonde_vwind, v:ht/1000.0},dlimit=dlimit
      options,'iug_radiosonde_bdg_vwnd',ytitle='RSND-bdg!CHeight!C[km]',ztitle='vwnd!C[m/s]'
      options, ['iug_radiosonde_bdg_press','iug_radiosonde_bdg_temp',$
                'iug_radiosonde_bdg_rh',$
                'iug_radiosonde_bdg_uwnd','iug_radiosonde_bdg_vwnd'], 'spec', 1
   endif 
  ...
  ;---Add tdegap and ylim, zlim
   new_vars=tnames('iug_radiosonde_*')
   if new_vars[0] ne '' then begin  
      tdegap, 'iug_radiosonde_bdg_press',dt=10800*2,/overwrite
      tdegap, 'iug_radiosonde_bdg_temp',dt=10800*2,/overwrite
      tdegap, 'iug_radiosonde_bdg_rh',dt=10800*2,/overwrite
      tdegap, 'iug_radiosonde_bdg_uwnd',dt=10800*2,/overwrite
      tdegap, 'iug_radiosonde_bdg_vwnd',dt=10800*2,/overwrite
      zlim,'iug_radiosonde_bdg_uwnd',-40,40
      zlim,'iug_radiosonde_bdg_vwnd',-20,20
   endif
endif 
