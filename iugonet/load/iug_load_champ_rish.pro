...
pro iug_load_champ_rish, min_lat = min_lat, $
  ...
  if ~size(fns,/type) then begin
    ...
    source.local_data_dir = root_data_dir() + 'iugonet/rish/champ/fsi/v1_0/'
    source.remote_data_dir = 'http://database.rish.kyoto-u.ac.jp/arch/iugonet/data/GPS/champ/fsi/v1_0/nc/'
  ...
  if (downloadonly eq 0) then begin
    ...
    for j=0L,n_elements(local_paths)-1 do begin
      file= local_paths[j]
      if file_test(/regular,file) then  dprint,'Loading the GPS-RO CHAMP FSI data: ',file $
      else begin
        dprint,'The GPS-RO CHAMP FSI data ',file,' not found. Skipping'
        continue
      endelse
    ...
    if size(event_app,/type) eq 4 then begin 
       ...
       store_data,'gps_ro_champ_fsi_event',data = {x:time_app, y:event_app},dlimit=dlimit
       options, 'gps_ro_champ_fsi_event', ytitle = 'Event number',spec=0
    
       store_data,'gps_ro_champ_fsi_gpsid',data = {x:time_app, y:gpsid_app},dlimit=dlimit       
       options, 'gps_ro_champ_fsi_gpsid', ytitle = 'GPS satellite ID',spec=0
    
       store_data,'gps_ro_champ_fsi_leoid',data = {x:time_app, y:leoid_app},dlimit=dlimit
       options, 'gps_ro_champ_fsi_leoid', ytitle = 'LEO satellite ID',spec=0
    
       store_data,'gps_ro_champ_fsi_lat',data = {x:time_app, y:lat_app},dlimit=dlimit
       options, 'gps_ro_champ_fsi_lat', ytitle = 'Latitude [degree]',spec=0
    
       store_data,'gps_ro_champ_fsi_lon',data = {x:time_app, y:lon_app},dlimit=dlimit
       options, 'gps_ro_champ_fsi_lon', ytitle = 'Longitude [degree]',spec=0
    
       store_data,'gps_ro_champ_fsi_ref',data = {x:time_app, y:ref_app, v:height},dlimit=dlimit
       options, 'gps_ro_champ_fsi_ref', ytitle = 'Height [km]', ztitle = 'Refractivity [N]',spec=1,/noiso
    
       store_data,'gps_ro_champ_fsi_pres',data = {x:time_app, y:pres_app, v:height},dlimit=dlimit
       options, 'gps_ro_champ_fsi_pres', ytitle = 'Height [km]', ztitle = 'Dry air pressure [hPa]',spec=1,/noiso
    
       store_data,'gps_ro_champ_fsi_temp',data = {x:time_app, y:temp_app, v:height},dlimit=dlimit
       options, 'gps_ro_champ_fsi_temp', ytitle = 'Height [km]',ztitle = 'Dry air temperature [degree C]',spec=1,/noiso
    
       store_data,'gps_ro_champ_fsi_tan_lat',data = {x:time_app, y:tan_lat_app, v:height},dlimit=dlimit
       options, 'gps_ro_champ_fsi_tan_lat', ytitle = 'Height [km]',ztitle = 'Latitude of perigee point [degree]',spec=1,/noiso
    
       store_data,'gps_ro_champ_fsi_tan_lon',data = {x:time_app, y:tan_lon_app, v:height},dlimit=dlimit
       options, 'gps_ro_champ_fsi_tan_lon',ytitle = 'Height [km]', ztitle = 'Longitude of perigee point [degree]',spec=1,/noiso

      ;---Specify the height range of each parameter:
       ylim,'gps_ro_champ_fsi_ref',0,40
       ylim,'gps_ro_champ_fsi_pres',0,40
       ylim,'gps_ro_champ_fsi_temp',0,40
       ylim,'gps_ro_champ_fsi_tan_lat',0,40
       ylim,'gps_ro_champ_fsi_tan_lon',0,40
       
      ;---Add tdegap 
       tdegap,'gps_ro_champ_fsi_ref',/overwrite
       tdegap,'gps_ro_champ_fsi_pres',/overwrite
       tdegap,'gps_ro_champ_fsi_temp',/overwrite
       tdegap,'gps_ro_champ_fsi_tan_lat',/overwrite
       tdegap,'gps_ro_champ_fsi_tan_lon',/overwrite
     endif
  endif
