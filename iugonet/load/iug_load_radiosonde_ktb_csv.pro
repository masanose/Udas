...
;  of the troposphere in csv format taken by the radiosonde at Koto. Tabang 
;  and loads data into tplot format.
;
;SYNTAX:
; iug_load_radiosonde_ktb_csv, downloadonly=downloadonly, trange=trange, verbose=verbose
;
;KEYWOARDS:
...
pro iug_load_radiosonde_ktb_csv, downloadonly=downloadonly, $
  trange=trange, $
  verbose=verbose
...
if ~size(fns,/type) then begin       
  ...
  ;===============================
   ...
   source.local_data_dir =  root_data_dir() + 'iugonet/rish/misc/ktb/radiosonde/csv/'
   source.remote_data_dir = 'http://database.rish.kyoto-u.ac.jp/arch/iugonet/sonde/data/kototabang/csv/'
...
if (downloadonly eq 0) then begin
  ...
  ;==============
   for j=0L,n_elements(local_paths)-1 do begin
      file= local_paths[j] 
      if file_test(/regular,file) then  dprint,'Loading Koto. Tabang sonde data file: ',file $
      else begin
         dprint,'Koto. Tabang sonde data file',file,'not found. Skipping'
         continue
      endelse
  ...
  ;---Add tdegap and ylim, zlim
   ...
   if new_vars[0] ne '' then begin  
      tdegap, 'iug_radiosonde_ktb_press',dt=10800,/overwrite
      tdegap, 'iug_radiosonde_ktb_temp',dt=10800,/overwrite
      tdegap, 'iug_radiosonde_ktb_rh',dt=10800,/overwrite
      tdegap, 'iug_radiosonde_ktb_uwnd',dt=10800,/overwrite
      tdegap, 'iug_radiosonde_ktb_vwnd',dt=10800,/overwrite
