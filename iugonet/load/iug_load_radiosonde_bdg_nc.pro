...
;  of the troposphere taken by the radiosonde at Bandung 
...
pro iug_load_radiosonde_bdg_nc, downloadonly=downloadonly, $
  trange=trange, $
  verbose=verbose
...
if ~size(fns,/type) then begin
  ...
  ;===============================
   ...
   source.local_data_dir =  root_data_dir() + 'iugonet/rish/misc/bdg/radiosonde/nc/'
   source.remote_data_dir = 'http://database.rish.kyoto-u.ac.jp/arch/iugonet/sonde/data/bandung/nc/'
...
if (downloadonly eq 0) then begin
  ...
  ;==============
   for j=0L,n_elements(local_paths)-1 do begin
      file= local_paths[j] 
      if file_test(/regular,file) then  dprint,'Loading Bandung radiosonde file: ',file $
      else begin
         dprint,'Bandung radiosonde data file',file,'not found. Skipping'
         continue
      endelse
   ...
   if size(sonde_press,/type) eq 4 then begin 
     ;---Create tplot variables and options
      ...
      options, ['iug_radiosonde_bdg_press','iug_radiosonde_bdg_temp',$
                'iug_radiosonde_bdg_rh','iug_radiosonde_bdg_dewp',$
