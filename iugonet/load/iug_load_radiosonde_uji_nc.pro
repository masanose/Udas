...
;  of the troposphere taken by the radiosonde at Uji 
;  and loads data into tplot format.
;
;SYNTAX:
; iug_load_radiosonde_uji_nc, downloadonly=downloadonly, trange=trange, verbose=verbose
;
;KEYWOARDS: 
...
pro iug_load_radiosonde_uji_nc, downloadonly=downloadonly, $
  trange=trange, $
  verbose=verbose
...
if ~size(fns,/type) then begin
  ...
  ;===============================
   ...
   source.local_data_dir =  root_data_dir() + 'iugonet/rish/misc/uji/radiosonde/nc/'
   source.remote_data_dir = 'http://database.rish.kyoto-u.ac.jp/arch/iugonet/sonde/data/uji/nc/'
...
if (downloadonly eq 0) then begin
  ...
  ;==============
   for j=0L,n_elements(local_paths)-1 do begin
      file= local_paths[j] 
      if file_test(/regular,file) then  dprint,'Loading Uji radiosonde file: ',file $
      else begin
         dprint,'Uji radiosonde data file ',file,'not found. Skipping'
         continue
      endelse
   ...
   if size(sonde_press,/type) eq 4 then begin 
     ;---Create tplot variables and options
      ...
      options, ['iug_radiosonde_uji_press','iug_radiosonde_uji_temp',$
                'iug_radiosonde_uji_rh','iug_radiosonde_uji_dewp',$
  ...
  ;---Add tdegap and zlim
   ...
   if new_vars[0] ne '' then begin  
      dt = 10800
      tdegap, 'iug_radiosonde_uji_press',dt=10800*2,/overwrite
      tdegap, 'iug_radiosonde_uji_temp',dt=10800*2,/overwrite
      tdegap, 'iug_radiosonde_uji_rh',dt=10800*2,/overwrite
      tdegap, 'iug_radiosonde_uji_uwnd',dt=10800*2,/overwrite
      tdegap, 'iug_radiosonde_uji_vwnd',dt=10800*2,/overwrite
