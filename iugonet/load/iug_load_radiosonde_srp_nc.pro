...
;  of the troposphere taken by the radiosonde at Serpong 
;  and loads data into tplot format.
;
;SYNTAX:
; iug_load_radiosonde_srp_nc, downloadonly=downloadonly, trange=trange, verbose=verbose
;
;KEYWOARDS: 
...
pro iug_load_radiosonde_srp_nc, downloadonly=downloadonly, $
  trange=trange, $
  verbose=verbose
...
if ~size(fns,/type) then begin
  ...
  ;===============================
   ...
   source.local_data_dir =  root_data_dir() + 'iugonet/rish/misc/srp/radiosonde/nc/'
   source.remote_data_dir = 'http://database.rish.kyoto-u.ac.jp/arch/iugonet/sonde/data/serpong/nc/'
...
if (downloadonly eq 0) then begin
  ...
  ;==============
   for j=0L,n_elements(local_paths)-1 do begin
      file= local_paths[j] 
      if file_test(/regular,file) then  dprint,'Loading Serpong radiosonde file: ',file $
      else begin
         dprint,'Serpong radiosonde data file ',file,'not found. Skipping'
         continue
      endelse
  ...
  ;---Acknowlegment string (use for creating tplot vars)
   ...
   if size(sonde_press,/type) eq 4 then begin 
     ;---Create tplot variables and options
      ...
      store_data,'iug_radiosonde_srp_press',data={x:sonde_time, y:sonde_press, v:ht/1000.0},dlimit=dlimit
      options,'iug_radiosonde_srp_press',ytitle='RSND-srp!CHeight!C[km]',ztitle='Press.!C[hPa]'
      store_data,'iug_radiosonde_srp_temp',data={x:sonde_time, y:sonde_temp, v:ht/1000.0},dlimit=dlimit
      options,'iug_radiosonde_srp_temp',ytitle='RSND-srp!CHeight!C[km]',ztitle='Temp.!C[deg.]'
      store_data,'iug_radiosonde_srp_rh',data={x:sonde_time, y:sonde_rh, v:ht/1000.0},dlimit=dlimit
      options,'iug_radiosonde_srp_rh',ytitle='RSND-srp!CHeight!C[km]',ztitle='RH!C[%]'
      store_data,'iug_radiosonde_srp_uwnd',data={x:sonde_time, y:sonde_uwind, v:ht/1000.0},dlimit=dlimit
      options,'iug_radiosonde_srp_uwnd',ytitle='RSND-srp!CHeight!C[km]',ztitle='uwnd!C[m/s]'
      store_data,'iug_radiosonde_srp_vwnd',data={x:sonde_time, y:sonde_vwind, v:ht/1000.0},dlimit=dlimit
      ...
      options, ['iug_radiosonde_srp_press','iug_radiosonde_srp_temp',$
                'iug_radiosonde_srp_rh','iug_radiosonde_srp_dewp',$
  ...
  ;---Add tdegap and zlim
   ...
   if new_vars[0] ne '' then begin  
      dt = 10800
      tdegap, 'iug_radiosonde_srp_press',dt=10800*2,/overwrite
      tdegap, 'iug_radiosonde_srp_temp',dt=10800*2,/overwrite
      tdegap, 'iug_radiosonde_srp_rh',dt=10800*2,/overwrite
      tdegap, 'iug_radiosonde_srp_uwnd',dt=10800*2,/overwrite
      tdegap, 'iug_radiosonde_srp_vwnd',dt=10800*2,/overwrite
