...
pro iug_load_radiosonde_pon_csv, downloadonly=downloadonly, $
  trange=trange, $
  verbose=verbose
...
if ~size(fns,/type) then begin       
  ...
  ;===============================
   ...
   source.local_data_dir =  root_data_dir() + 'iugonet/rish/misc/pon/radiosonde/csv/'
   source.remote_data_dir = 'http://database.rish.kyoto-u.ac.jp/arch/iugonet/sonde/data/pontianak/csv/'
...
if (downloadonly eq 0) then begin
  ...
  ;==============
   for j=0L,n_elements(local_paths)-1 do begin
      file= local_paths[j] 
      if file_test(/regular,file) then  dprint,'Loading Pontianak sonde data file: ',file $
      else begin
         dprint,'Pontianak sonde data file',file,'not found. Skipping'
         continue
      endelse
  ...
  ;---Add tdegap and ylim, zlim
   ...
   if new_vars[0] ne '' then begin  
      tdegap, 'iug_radiosonde_pon_press',dt=10800,/overwrite
      tdegap, 'iug_radiosonde_pon_temp',dt=10800,/overwrite
      tdegap, 'iug_radiosonde_pon_rh',dt=10800,/overwrite
      tdegap, 'iug_radiosonde_pon_uwnd',dt=10800,/overwrite
      tdegap, 'iug_radiosonde_pon_vwnd',dt=10800,/overwrite
