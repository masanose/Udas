   ...
   'OPEN_TEMPLATE': BEGIN
    ...
    'SAVEDATAAS': BEGIN
       ...
       IF Is_Num(data) THEN info.statusBar->Update, 'There is no data to save. Please load data using the Load THEMIS Data option under the File pull down menu.' $
         ELSE BEGIN 
         thm_ui_save_data_as, info.master, info.loadedData, info.historywin,info.guiTree, info.saveDataDirPtr, info.statusbar
         ENDELSE
    END
...
;        Tracking=tracking, Locked=locked)
    ...
    'CONF': BEGIN
      ...
      info.template_filename = !THM_GUI_NEW.templatepath
      !THM_GUI_NEW.windowstorage->GetProperty,template = templateobj
...
;Note on code related to various tracking switches
   ...
   'TRACKH': BEGIN
    ...
    'ZAXIS': thm_ui_zaxis_options, info.master, info.windowStorage, info.zAxisSettings, info.drawObject, info.loadedData, info.historywin,info.template_object, info.statusBar
    ...
    'SLICE': thm_ui_slice2d, gui_ID=info.master, historywin=info.historyWin    
           ...
           FOR i=0,N_Elements(windowObjs)-1 DO BEGIN
              ...
              IF name EQ names[index] THEN BEGIN
                 IF result EQ 0 THEN begin
                   ...
                   thm_ui_orientation_update,info.drawObject,info.windowStorage, rezoom=rezoom
                 endif 
              ENDIF
           ENDFOR
        ENDIF
...
;  markerPMenu = Widget_Button(graphMenu, Value='Marker Options... ', UValue='MARKERP')
  ...
  dprocMenu = Widget_Button(analysisMenu, Value = 'Data Processing... ', UValue = 'DPROC', $
                            sensitive = 1)
  slicesMenu = widget_button(analysisMenu, value='Velocity Slices... ', uval='SLICE')
...
;    Sensitive=0, UValue='REDO')
  ...
  manageBmpButton = Widget_Button(dataToolBase, Value=manageBMP, /Bitmap, ToolTip='Manage GUI Data and Import and Export Tplot data', $
    UValue='MANAGEDATA')
  zoomSpinner = thm_ui_spinner(zoomBase, Increment=10, Value=100, uval='ZOOM',UNITS="%", min_value=1, max_value=200)
  ...
  ;read template from file, if requested or read template if stored in !thm_gui_new.templatepath (obtained from config file) - give priority to keyword if passed in
  if keyword_set(template_filename) then begin
    ...
    endelse
  endif else if thm_exists && in_set(strlowcase(tag_names(!THM_GUI_NEW)),'templatepath') && (size(!THM_GUI_NEW.templatepath,/type) eq 7) then begin
    if !THM_GUI_NEW.templatepath ne '' then begin
      open_themis_template,template=template,filename=!THM_GUI_NEW.templatepath,$
        statusmsg=statusmsg,statuscode=statuscode
      if statuscode lt 0 then begin
        ok = dialog_message(statusmsg,/error,/center)
        template=obj_new()
      endif else begin
        template_file = !THM_GUI_NEW.templatepath
      endelse
    endif
  endif
...
;   if ~!journal then Journal,'thm_ui_idlsave.pro'
  ...
  info = {master:master,$ ;The ID of the top widget base for the entire GUI
     ...
     ;     markerTitle:markerTitle, $
          ...
          oplot_calls:!THM_GUI_NEW.oplot_calls,$
          contextMenuOn:0,$
  ...
  scrollbar = obj_new('THM_UI_SCROLL_BAR', scrollbase, x_scr_size, $
