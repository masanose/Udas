...
;Keywords:
;  DATATYPE (I/O):
;    Set to 'iono_fr_dpl'.  If not set, 'iono_fr_dpl' is
;      assumed.  Returns cleaned input, or shows default. 
;  PARAMETERS (I/O):
;    Set to wind parameters.  If not set, 'fr_dpl_beam1' is
;      assumed.  Returns cleaned input, or shows default.       
;  TRANGE (In):
...
pro iug_load_ear_iono_fr, datatype=datatype, parameters=parameters, trange=trange, verbose=verbose
...
;************************************
if ~keyword_set(parameters) then parameters='fr_dvr_beam1'
...
if ~size(fns,/type) then begin
    ...
    if datatype eq 'iono_fr_dpl' then begin
      file_names = file_dailynames( $
        file_format='YYYY/'+$
        'YYYYMMDD',trange=trange,times=times,/unique)+'.faifb1p16d.dpl'+vdcnames+'.csv'
    endif else if datatype eq 'iono_fr_pwr' then begin
      file_names = file_dailynames( $
        file_format='YYYY/'+$
        'YYYYMMDD',trange=trange,times=times,/unique)+'.faifb1p16d.pwr'+vdcnames+'.csv'
    endif else if datatype eq 'iono_fr_spec_width' then begin
      file_names = file_dailynames( $
        file_format='YYYY/'+$
        'YYYYMMDD',trange=trange,times=times,/unique)+'.faifb1p16d.wdt'+vdcnames+'.csv'
    endif else if datatype eq 'iono_fr_noise_lev' then begin
      file_names = file_dailynames( $
        file_format='YYYY/'+$
        'YYYYMMDD',trange=trange,times=times,/unique)+'.faifb1p16d.pn'+vdcnames+'.csv'
    endif
    ;
