...
;
;Syntax:
; iug_load_meteor_ser [ ,DATATYPE = string ]
;                     [ ,PARAMETERS = string]
;                     [ ,TRANGE = [min,max] ]
...
pro iug_load_meteor_ser, datatype=datatype, parameters = parameters, trange=trange, verbose=verbose
...
if ~size(fns,/type) then begin
    ...
    source.local_data_dir =  root_data_dir() + 'iugonet/rish/serpong/winddata/h2km_t60min00/'
...
;=============================================
if  parameters eq 'zonal_wind_ser' then begin
    dlimit=create_struct('data_att',create_struct('acknowledgment',acknowledgstring))
    store_data,'zonal_wind_ser',data={x:se_time, y:zon_wind, v:height},dlimit=dlimit
endif
if  parameters eq 'meridional_wind_ser' then begin
    dlimit=create_struct('data_att',create_struct('acknowledgment',acknowledgstring))
    store_data,'meridional_wind_ser',data={x:se_time, y:mer_wind, v:height},dlimit=dlimit
endif

;Store data of zonal and meridional thermal speed component:
;===========================================================
if  parameters eq 'zonal_thermal_speed_ser' then begin
    dlimit=create_struct('data_att',create_struct('acknowledgment',acknowledgstring))
    store_data,'zonal_thermal_speed_ser',data={x:se_time, y:zon_thermal, v:height},dlimit=dlimit
endif
if  parameters eq 'meridional_thermal_speed_ser' then begin
    dlimit=create_struct('data_att',create_struct('acknowledgment',acknowledgstring))
    store_data,'meridional_thermal_speed_ser',data={x:se_time, y:mer_thermal, v:height},dlimit=dlimit
endif
