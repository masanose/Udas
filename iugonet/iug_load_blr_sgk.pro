...
;
;Syntax:
; iug_load_blr_sgk  [ ,DATATYPE = string ]
;                   [ ,PARAMETERS = string ]
;                   [ ,TRANGE = [min,max] ]
...
;Keywords:
;  DATATYPE (I/O):
;    Set to datatype.  If not set, 'blr_wind_sgk' is
;      assumed.  Returns cleaned input, or shows default. 
;  PARAMETERS (I/O):
;    Set to wind parameters.  If not set, 'sgk_wind_zon' is
;      assumed.  Returns cleaned input, or shows default.       
;  TRANGE (In):
...
pro iug_load_blr_sgk, datatype=datatype, parameters=parameters, trange=trange, verbose=verbose
...
;****************************************
if ~keyword_set(datatype) then datatype='blr_wind_sgk'

;************************************
;Load wind component data by default:
;************************************
if ~keyword_set(parameters) then parameters='sgk_zonal_wind'
...
endif else if datatype eq 'blr_spec_width_sgk' then begin
...
;================================
 for i=0,n_elements(local_paths)-1 do begin
    file= local_paths[i]
    if file_test(/regular,file) then  dprint,'Loading BLR-Shigaraki file: ',file $
    else begin
