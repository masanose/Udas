...
;
;Syntax:
; iug_load_meteor_srp_nc [ ,DATATYPE = string ]
;                        [ ,PARAMETERS = string]
;                        [ ,TRANGE = [min,max] ]
...
pro iug_load_meteor_srp_nc, datatype=datatype, parameters = parameters, trange=trange, verbose=verbose
...
if ~size(fns,/type) then begin
    ...
    ;=========GUI mode=======================================================
    file_names = file_dailynames( $
      file_format='YYYYMM',trange=trange,times=times,/unique)+'_serp.nc'
    ;=========================================================================
    ;        
    ...
    source.local_data_dir =  root_data_dir() + 'iugonet/rish/mw/serpong/serp_wind_h2km_t60min00netCDF/'
...
;======================================
 for j=0,n_elements(local_paths)-1 do begin
    ...
    height = fltarr(36)
    uwind_srp=fltarr(n_elements(time),36)
    vwind_srp=fltarr(n_elements(time),36)
    sig_uwind_srp=fltarr(n_elements(time),36)
    sig_vwind_srp=fltarr(n_elements(time),36)
    num_srp=fltarr(n_elements(time),36)
    ...
    for i=0, n_elements(time)-1 do begin
        ...
        for k=0, 35 do begin
            ...
            uwind_srp[i,k]=uwind[k+36*i]
            vwind_srp[i,k]=vwind[k+36*i]
            sig_uwind_srp[i,k]=sig_uwind[k+36*i]
            sig_vwind_srp[i,k]=sig_vwind[k+36*i]
            num_srp[i,k]=num[k+36*i]
   ...
   ;======================================
    append_array, srp_time, timeunix
    append_array, zon_wind, uwind_srp
    append_array, mer_wind, vwind_srp
    append_array, zon_thermal, sig_uwind_srp
    append_array, mer_thermal, sig_vwind_srp
    append_array, meteor_num, num_srp
...
;=============================================
   if  parameters eq 'zonal_wind_srp' then begin
       ...
       store_data,'zonal_wind_srp',data={x:srp_time, y:zon_wind, v:height},dlimit=dlimit
   endif
   if  parameters eq 'meridional_wind_srp' then begin
       dlimit=create_struct('data_att',create_struct('acknowledgment',acknowledgstring))
       store_data,'meridional_wind_srp',data={x:srp_time, y:mer_wind, v:height},dlimit=dlimit
   endif
...
;===========================================================
   if  parameters eq 'zonal_thermal_speed_srp' then begin
       dlimit=create_struct('data_att',create_struct('acknowledgment',acknowledgstring))
       store_data,'zonal_thermal_speed_srp',data={x:srp_time, y:zon_thermal, v:height},dlimit=dlimit
   endif
   if  parameters eq 'meridional_thermal_speed_srp' then begin
       dlimit=create_struct('data_att',create_struct('acknowledgment',acknowledgstring))
       store_data,'meridional_thermal_speed_srp',data={x:srp_time, y:mer_thermal, v:height},dlimit=dlimit
   endif
;Store data of meteor trace number:
;==================================
   if  parameters eq 'meteor_num_srp' then begin
       dlimit=create_struct('data_att',create_struct('acknowledgment',acknowledgstring))
       store_data,'meteor_num_srp',data={x:srp_time, y:meteor_num, v:height},dlimit=dlimit
   endif
