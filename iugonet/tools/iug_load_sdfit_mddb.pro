  ...
  IF ~KEYWORD_SET(cdffn) THEN BEGIN
    ...
    if n_elements(stns) gt 1 then begin
      for i=0, n_elements(stns)-1 do begin
        iug_load_sdfit_mddb, sites=stns[i], get_support_data=get_support_data,$
          noacknowledgment=noacknowledgment, trange=trange,$
          downloadonly=downloadonly, no_download=no_download
      endfor
      return
    ...
    ;Get the time range
    get_timespan, tr
    tr[1] -= 0.0001 ; put the end of time range within the previous day
    ...
    if ~keyword_set(no_mddb) then begin
      ...
      keyword='superdarn+AND+'+stn+'+AND+cdf'
      startdate = tr_str[0]
      enddate = tr_str[1]
      ...
      IF N_ELEMENTS(lst) EQ 0 AND STRLEN(lst[0]) EQ 0 THEN BEGIN
        PRINT, 'iug_load_sdfit_mddb: No data file was hit by MDDB query!'
        return
      ENDIF
      lst = lst[ SORT(lst) ] ;Sort the URL list
      
      nlst = n_elements(lst)
      relfnames = strarr(nlst)
      for i_lst=0L, nlst-1 do begin
        url = lst[i_lst]
        url_cmp = strsplit( url, '/', /extract )
        n = n_elements(url_cmp)
        relfnames[i_lst] = url_cmp[n-2] + '/' + url_cmp[n-1]
      endfor
      ;;print, relfnames & return ;for debugging
