...
; $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/ssl_general/tags/tdas_5_21/misc/quaternion/qslerp.pro $
;-
...
if(size(qn,/n_dim) eq 0 && qn[0] eq -1) then begin
  ...
  message, /continue, 'Unable to calculate norm of input quaternions'
...
if(idx[0] ne -1) then begin
  ...
  message, /continue, 'At least one input quaternion is not unit length'
...
if(qdims[0] ne n_elements(x1i)) then begin
  ...
  message, /continue, 'Number of input abcissa values does not match the number of input quaternions'
...
if(n_elements(x1i) gt 1) then begin 
   ...
   if(idx[0] ne -1) then begin
      ...
      message, /continue, 'input abcissa values not monotonic'
...
;check that output abcissa values are strictly monotonic
   ...
   if(idx[0] ne -1) then begin
      ...
      message, /continue, 'output abcissa values not monotonic'
...
idx = where(x2i ge x1i[0] and x2i le x1i[n_elements(x1i)-1])

if(idx[0] eq -1) then return,reform(q_out)
...
if(idx[0] ne -1) then begin
  ...
  q_out[idx,*] = qi[q_idx[idx],*] 

endif

out_idx = where(abs(t_list) gt EQ_TOLERANCE) ;where t_list !=~ 0.0
