...
function ssl_set_complement, l1, l2
  ...
  if(size(l2, /n_dim) eq 0) then begin 
    message,/info, 'argument l2 to ssl_set_complement should be of type array'
    return, -1L
  endif

  if(size(l1, /n_dim) eq 0) then begin
    message,/info, 'argument l1 to ssl_set_complement should be of type array'
    return, -1L
  endif
