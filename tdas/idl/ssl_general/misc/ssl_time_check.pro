...
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/ssl_general/tags/tdas_5_21/misc/ssl_time_check.pro $
;-

;SUB-FUNCTION:     thm_variable_defined
...
function thm_variable_defined, file, mnem
...
;print, cdf_in.vars[125].name

;iterate over variables
for i = 0, cdf_in.nv-1 do begin

  ;print,cdf_in.vars[i-1].name	

  if cdf_in.vars[i].name eq mnem && cdf_in.vars[i].numrec gt 1 then begin

    ;print,cdf_in.vars[i].name, i, cdf_in.vars[i].numrec
...
;   none
;
;EXAMPLE: c = thm_union(a,b)
;
;CREATED BY:    Patrick Cruce (pcruce@gmail.com)

function thm_union, a, b
...
;SUB-FUNCTION: thm_discrete_derivative
...
;   none
;
;EXAMPLE: thm_discrete_derivative,a,b
;
;CREATED BY:    Patrick Cruce (cruce@gmail.com)

pro thm_discrete_derivative, a, b
...
;SUB-PROCEDURE:     thm_write_derivative_limits
...
;   none
;
;EXAMPLE: thm_write_derivative_limits,file,a,lim
;
;CREATED BY:    Patrick Cruce (pcruce@gmail.com)

pro thm_write_derivative_limits, file, a, lim

get_lun, lun

openw, lun, file, /APPEND

thm_discrete_derivative, a, b
...
;SUB-PROCEDURE:     thm_get_mnem_file
...
;   none
;
;EXAMPLE: thm_get_mnem_file,'filename','mnem',a
;
;CREATED BY:    Patrick Cruce (pcruce@gmail.com)


pro thm_get_mnem_file, filename, mnem, a
...
for i = 1, in.nv do begin
  ...
  if stregex(in.vars[i-1].name, mnem, /BOOLEAN) && in.vars[i-1].numrec gt 0 then begin
    ...
    ;print, in.vars[i-1].name, i-1, in.vars[i-1].numrec
...
;   none
;
;EXAMPLE: thm_time_list,'filename'
;
;CREATED BY:    Patrick Cruce (pcruce@gmail.com)

pro thm_time_list, filename, in
...
for i = 1, cdf_in.nv do begin
  ...
  if stregex(cdf_in.vars[i-1].name, '.*_time', /BOOLEAN) && cdf_in.vars[i-1].numrec gt 0 then begin
    ...
    ;print, cdf_in.vars[i-1].name, i-1, cdf_in.vars[i-1].numrec]
    ...
    ;print, 'Got Values Plotting'

    ;plot them
    ;plot, values_d

    ;print, 'Plotted Press .C to continue'
...
;SUB-PROCEDURE:     thm_write_lims_dir
...
;   none
;
;EXAMPLE: thm_write_lims_dir,'in_directory','out_directory','mnem_name',[0.0,1.0]
;
;CREATED BY:    Patrick Cruce (pcruce@gmail.com)

pro thm_write_lims_dir, dir, mnem, lim, out
...
for i = 0, (N_ELEMENTS(s)-1) do begin
  ...
  if thm_variable_defined(s[i], mnem) then begin
  
    ;get the values from a given file
    thm_get_mnem_file, s[i], mnem, v
    ...
    thm_discrete_derivative, b, c

    spl = strsplit(s[i], '/', /EXTRACT)

    thm_write_derivative_limits, out+'/'+mnem+'.lim', b, lim
...
;SUB-PROCEDURE:     thm_time_list_dir
...
;   none
;
;EXAMPLE: thm_time_list_dir,'dirname'
;
;CREATED BY:    Patrick Cruce (pcruce@gmail.com)

pro thm_time_list_dir, list, in
  
  thm_time_list, list[0], in

  for i = 1, N_ELEMENTS(list)-1 do begin
      
    thm_time_list, list[i], out
...
for i = 1,(n_elements(a)-1 ) do begin
  ...
  if(stregex(a[i], mnem, /BOOLEAN)) then begin
    ...
    thm_write_lims_dir, dir, a[i], lim, out
