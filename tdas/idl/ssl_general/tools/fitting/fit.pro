...
;
; PROCEDURE:
;       Copied from "CURVEFIT", least squares fit to a non-linear
...
; SET ALL DEFAULTS:
       ...
       dprint,verbose=verbose,dlevel=1,derstr[NODERIVATIVE]
...
;       if not keyword_set(verbose) then $
          dprint,dlevel=1,"P_NAMES keyword is replaced by NAMES"
   endif
...
;            Evaluate function and estimate partial derivatives
          ...
          mp = (nterms < maxprint)-1
              if nterms ne nterms_last then dprint,verbose=verbose,dlevel=1,'Chi',fullnames[0:mp],format=nformat
              nterms_last = nterms
          ...
          if nfree lt 0 then dprint,verbose=verbose, 'Not enough data points.'
...
;         Evaluate alpha and beta matricies.
          ...
          dprint,verbose=verbose,dlevel=2,iter,sqrt(chisq1),a[0:mp],format=vformat
...
if finite(chisqr) eq 0 then begin
    qflag = 4                   ;added ajh
    dprint,verbose=verbose,'Invalid Data or Parameters in function '+function_name_com+' Aborting'
    dprint,verbose=verbose,iter,sqrt(chisq1),b[0:mp],format=vformat
...
if flambda ge 1e5 then  begin
    qflag = 2                   ;added ajh
    dprint,verbose=verbose,'flambda too large (',flambda,') for ',function_name_com,' Aborting'
    dprint,verbose=verbose,iter,sqrt(chisq1),b[0:mp],format=vformat
...
;       IF iter NE itmax+1 THEN qflag = 1 ;added ajh
       dprint,verbose=verbose,dlevel=1,iter+1,sqrt(chi2),a[0:mp],format=vformat
       dprint,verbose=verbose,dlevel=1,sqrt(chi2),sigma[0:mp],format=sformat
       dprint,verbose=verbose,dlevel=1,'Chi2 =',chi2,'  flambda =',flambda
