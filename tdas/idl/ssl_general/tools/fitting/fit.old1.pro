...
;
; CALLING SEQUENCE:
;       FIT,X, Y, PARAMETERS=par, P_NAMES=strarray, $
...
; SET ALL DEFAULTS:
       ...
       derstr = 'Partial derivatives computed '+ [ 'analytically','numerically' ] $
            +' for function: '+strupcase(function_name_com)
...
;            Evaluate function and estimate partial derivatives
          ...
          if not keyword_set(testname) then begin
             ...
             if npdernz ne nterms then begin
                 pder = pder[*,wpdernz]
                 a = a[wpdernz]
                 fullnames = fullnames[wpdernz]

                 ;p_names = fullnames
                 if not keyword_set(silent) then $
                    print,npdernz,nterms,format="('Warning:  Fitting to ',i0.0,' of ',i0.0,' chosen parameters')"
                 nterms = npdernz
;                 help,pder,nterms,a,fullnames
             endif
          endif
          mp = (nterms < maxprint)-1
          if not keyword_set(silent) then begin
              if nterms ne nterms_last then print,'Chisq',fullnames[0:mp],format=nformat
              nterms_last = nterms
          endif
...
if finite(chisqr) eq 0 then begin
    ...
    print,iter,chisq1,b[0:mp],format=vformat
...
;       sigma = sqrt(array[diag]/alpha[diag] * (1+flambda)) ; Return sigma's
       sigma = sqrt((invert(alpha/c))[diag]/alpha[diag] ) ; Return sigma's
       chi2 = chisqr                          ; Return chi-squared
;       IF iter NE itmax+1 THEN qflag = 1 ;added ajh
       if not keyword_set(silent) then begin
          print,iter+1,chi2,a[0:mp],format=vformat
          print,chi2,sigma[0:mp],format=sformat
