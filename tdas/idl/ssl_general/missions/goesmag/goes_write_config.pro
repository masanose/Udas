...
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/ssl_general/tags/tdas_5_21/missions/goesmag/goes_write_config.pro $
;-

Pro goes_write_config, copy = copy, _extra = _extra
  otp = -1
;First step is to get the filename
  dir = goes_config_filedir()
...
;Does the file exist? If is does then copy it
    ...
    filex_out = filex
    cfg = {local_data_dir:!goes.local_data_dir, $
           remote_data_dir:!goes.remote_data_dir, $
           no_download:!goes.no_download, $
           no_update:!goes.no_update, $
           downloadonly:!goes.downloadonly, $
           verbose:!goes.verbose}
    
           
    hhh = [';goes_config.txt', ';GOES configuration file', $
