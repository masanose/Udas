...
;Procedure: slr
;
;Purpose: This routine will generate SLR parse tables for a bottom-up shift/
;         reduce parcer.  The entire algorithm is described in "Compilers:
;         Principles, Tools, and Techniques" by Aho, Sethi, & Ullman section 
;         4.7
;         
...
;         Different versions of closure, goto, & items are needed for lalr, or lrk methods.
;         These have not been implmented
;         
...
; $LastChangedDate: 2009-07-08 17:53:08 -0700 (Wed, 08 Jul 2009) $
; $LastChangedRevision: 6401 $
; $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/ssl_general/tags/tdas_5_21/mini/slr.pro $
;-
          ...
          if goto_index[0] ne -1 then begin
            ...
            if action_table[i,element_index] ne '' && action_table[i,element_index] ne 's' + strtrim(string(goto_index),2) then begin
              ...
              if strmid(action_table[i,element_index],0,1) eq 's' then begin
                return,{type:'error',name:'action shift conflict',value:'conflict with state',state_num:i,state_symbols:state_symbols,goto_index:goto_index,element:element,element_index:element_index[0],action_table:action_table,set_of_items:set_of_items}
              endif
