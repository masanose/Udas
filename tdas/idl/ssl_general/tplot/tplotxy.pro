...
; $LastChangedDate: 2008-07-28 11:52:58 -0700 (Mon, 28 Jul 2008) $
; $LastChangedRevision: 3310 $
; $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/ssl_general/tags/tdas_5_21/tplot/tplotxy.pro $
;-

;main function
pro tplotxy, vectors, title=title,overplot=overplot,addpanel=addpanel,multi=multi, usetrange=usetrange,$
...
if n_elements(names) gt 1 then begin
   ...
   for i = 0,n_elements(names)-1 do begin
      ...
      if keyword_set(colors) then begin
       ...
       tplotxy, names[i], title=title,overplot=op,addpanel=add,multi=mlt,usetrange=usetrange,$
 ...
 if(size(d,/type) eq 7) then begin
    ...
    for i = 0,n_elements(d)-1 do begin
       ...
       tplotxy, d[i], title=title,overplot=op,addpanel=add,multi=mlt, usetrange=usetrange,$
...
plotxy,vecs,overplot=overplot,addpanel=addpanel,multi=multi,title=titl,colors=cols,units=uni,labels=labels,$
  xtick_get=xtick_get,ytick_get=ytick_get,_extra=_extra
