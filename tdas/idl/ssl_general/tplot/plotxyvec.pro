...
; $LastChangedDate: 2008-08-15 14:15:37 -0700 (Fri, 15 Aug 2008) $
; $LastChangedRevision: 3406 $
; $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/ssl_general/tags/tdas_5_21/tplot/plotxyvec.pro $
;
;-
...
pro plotxyvec,xy,dxy,noisotropic=noisotropic,addpanel=addpanel, $
              overplot=overplot,multi=multi,xrange=xrange,yrange=yrange,$
              ...
              color=color,replot=replot,_extra=ex
  ...
  if dimxy[n_elements(dimxy)-1] ne 2 || dimdxy[n_elements(dimdxy)-1] ne 2 || n_elements(dimxy) ne n_elements(dimdxy) then begin
    dprint,'xy & dxy must be Nx2 arrays'
 ...
 ; print,dsize,ascale,xyrange,dxyrange
  ...
  if ~keyword_set(overplot) then begin
     ...
     pos = pxy_get_pos(xr2,yr2,isotropic,xmargin,ymargin)
