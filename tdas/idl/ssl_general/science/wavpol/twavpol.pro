...
nopfft=nopfft,steplength=steplength
  ...
  if not keyword_set(tvarname) then begin
    message, /continue, 'tvarname must be set'
    return
  endif
  ...
  if(var_names[0] eq '') then begin
    message, /continue, 'No valid tplot_variables match tvarname'
    return
  endif
  ...
  if dimx[0] ne dimy[0] then begin
     message,/continue,'Number of time elements does not match number of magnetic field elements in tvarname'
     return
  endif

  if n_elements(dimy) ne 2 then begin
     message,/continue,'Data component of tvarname must be a 2 dimensional array'
     return
  endif

  if dimy[1] ne 3 then begin
     message,/continue,'Dimension 2 of data component of tvarname must have 3 elements'
     return
  endif

  wavpol, d.x, d.y[*, 0], d.y[*, 1], d.y[*, 2], timeline, freqline,$
   powspec, degpol, waveangle, elliptict, helict,nopfft=nopfft,steplength=steplength
