...
; $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/ssl_general/tags/tdas_5_21/cotrans/special/minvar/minvar_matrix_make.pro $
;-
...
if not keyword_set(in_var_name) then begin
    message,/continue,' fx requires in_var_name to be set'
    return
endif
...
if n_elements(d_s) ne 2 then begin
    message,/continue,' fx requires in_var_name.y to have 2 dimensions'
    return
endif

if d_s[1] ne 3 then begin
    message,/continue,' fx requires cardinality of the second dimensions of in_var_name.y to equal 3'
    return
endif
...
if(start_d ge stop_d) then begin
  message, /continue, 'fx requires tstart to be greater than or equal to tstop'
  return
endif
...
while(current_d + twindow le stop_d) do begin ;loop over time windows
  ...
  if(i ge o_num) then begin ;output storage should never exceed allocated space
    message, /continue, ' fx output array index out of bounds'
    return
  endif
  ...
  if(size(idx, /n_dim) eq 0 && idx eq -1L) then begin
    ...
    ;message,/continue, 'fx had where index error'
...
;the conditionals below should never fail, but it doesn't hurt to test
  if not array_equal(size(o_eig, /dimensions), [3, 3]) then begin
    message, /continue, 'minvar produced transformation matrix with wrong dimensions'
    return
  endif

  if not array_equal(size(o_lam, /dimensions), [3]) then begin
    message, /continue, 'minvar produced lambda matrix with wrong dimensions'
    return
  endif
