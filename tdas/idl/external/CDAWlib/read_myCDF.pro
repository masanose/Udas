...
;$Header: /home/cdaweb/dev/control/RCS/read_myCDF.pro,v 1.242 2009/11/06 17:11:14 kovalick Exp kovalick $
;$Locker: kovalick $
;$Revision: 7091 $
;+------------------------------------------------------------------------
...
;TJK 9/17/2009 try this instead - the following is much faster
       data = [[old],[new]]
    end
   3:begin
...
;TJK - try conserving memory and maybe speed things up?
;           new = reform(new)
;           old = reform(old)
;print, ' DEBUG in case 3 of append_mydata, but the vector case'
           new = reform(temporary(new))
           old = reform(temporary(old))
;TJK 9/17/2009 - replace the following 4 lines w/ the one to append 2-d array
;           a = size(new)
;           b = size(old)
;           data = make_array(a(1),a(2)+b(2),type=a(3))
;           data(*,0:b(2)-1)=old(*,*) & data(*,b(2):*)=new(*,*)
...
;print, ' DEBUG in case 3 of append_mydata'
;         data = make_array(a(1),a(2),a(3)+b(3),type=a(4))
;         data(*,*,0:b(3)-1)=old(*,*,*) & data(*,*,b(3):*)=new(*,*,*)
;TJK 9/14/2009 Can  do the same thing as above w/ the following (much faster)
        data = [[[old]],[[new]]]        
       endelse
    end
...
;                          rec_count)
            ...
            if (num_virs gt -1) then begin
               ...
               if (vv_cnt ge 1) then begin
                  ...
                  endelse
;print, 'Virtual variable found ',vnames(vx),' READ_FLAG BEFORE check for component_0 = ',read_flag 
;                  ; 11/5/2009 - another exception, if the data cdfs have inaccurately defined a
;                  ; variable as "virtual" when it isn't... check for component_0
;                  if (cdf_attexists(CDFid,'COMPONENT_0',vnames(vx)))then begin
;                     comp_num = cdf_attnum(CDFid,'COMPONENT_0')
;                     if (comp_num ne -1) then begin
;                       cdf_attget,CDFid,'COMPONENT_0',vnames(vx),parent_var
;                       if (n_elements(parent_var) gt 1) then read_flag = 0 else read_flag = 1
;                     endif
;                  endif else begin 
;                      read_flag = 1
;                  endelse
;print, 'Virtual variable found ',vnames(vx),' READ_FLAG AFTER check for component_0 = ',read_flag 
;print, 'If valid component_0 value not found, then switch the read_flag to 1 and attempt to read data'
		  ...
		  if (vnames(vx) eq 'FH') or $
		     ...
		     ; same case.
		        ...
		        valid_recs = where((epoch le stop_time) and $
			      (epoch ge start_time))
                        if keyword_set(DEBUG) then print, 'Recalculated - Reading ', n_elements(valid_recs), ' records.'
	             ;endif   
		     tmpdata=0
...
;DEBUG temptime = systime(1)
                  data = append_myDATA(data,olddata,dict_key=dk,vector=vector)  ; append new data to old data
;DEBUG total_storage_time = total_storage_time + (systime(1)-temptime)
...
;DEBUG print, 'It took an average of ',total_storage_time/cx,'seconds to append data'
