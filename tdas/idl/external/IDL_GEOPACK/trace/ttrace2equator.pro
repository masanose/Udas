...
; $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/idl_socware/tags/tdas_5_21/external/IDL_GEOPACK/trace/ttrace2equator.pro $
;-
...
pro ttrace2equator,in_pos_tvar,newname = newname, trace_var_name = $
...
km, par=par,period=period,error = error, r0=r0,rlim=rlim,noboundary=$

noboundary,storm=storm,pdyn=pdyn,dsti=dsti,yimf=yimf,zimf=zimf,g1=g1,$

g2=g2,w1=w1,w2=w2,w3=w3,w4=w4,w5=w5,w6=w6,get_tilt=get_tilt,set_tilt=set_tilt,$
...
if n_elements(set_tilt) gt 0 then begin
  ...
  if keyword_set(trace_tvar) then  begin
    ...
    trace2equator,d.x,d.y,f,$
    
    out_trace_array = tr,in_coord=in_coord,out_coord=out_coord,$
    
    internal_model=internal_model,external_model=external_model,south=$
    
    south,km=km,par=par_in,period=period,error=e,r0=r0,rlim=rlim,$
    
    get_nperiod=get_nperiod,get_tilt=tilt_dat,set_tilt=set_tilt_dat,$
      
    get_period_times=period_times_dat,_extra=_extra 
  
  endif else begin
  
    trace2equator,d.x,d.y,f,in_coord=in_coord,out_coord=$
    
    out_coord,internal_model=internal_model,external_model=external_model,$
    
    south=south,km=km,par=par_in,period=period,error=e,r0=r0,rlim=rlim,$
 ...
 if keyword_set(trace_tvar) then  begin
    ...
    get_nperiod=get_nperiod,get_tilt=tilt_dat,add_tilt=add_tilt_dat,$
 ...
 if keyword_set(trace_tvar) then  begin
    ...
    get_nperiod=get_nperiod,get_tilt=tilt_dat,$
