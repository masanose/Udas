...
; VERSION:
;   $LastChangedBy: hfrey $
;   $LastChangedDate: 2009-10-01 14:47:11 -0700 (Thu, 01 Oct 2009) $
;   $LastChangedRevision: 6795 $
;   $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/thmsoc/tags/tdas_5_21/idl/themis/ground/asi_mosaic/thm_mosaic_array.pro $
;-
...
; we will figure out the mask later, hfrey, 2009-09-30
  ...
  if w(0) ne -1 then begin
    ...
    if keyword_set(verbose) then for i=0,n_elements(w)-1 do print,'VERBOSE_COMMENT_01:  '+string(i+1,format='(i2.2)')+'  '+thg_date_and_site_string(i)
...
if keyword_set(verbose) then print,'After init: ',systime(1)-verbose,$
   ' Seconds'
...
if keyword_set(verbose) then print,'After stations: ',systime(1)-verbose,$
   ' Seconds'
...
;       if keyword_set(local) then f_img='local_images\'+date_string+'\'+date_and_hour_string+'_'+station_string+'*full.pgm*'
       ...
       if keyword_set(verbose) then print,'VERBOSE_COMMENT_03:'+'  '+$
            string(jtf_img,format='(i3.3)')+'  '+f_img
     ;--------------------------------------------------------------------------------------

	; some general debugging, keep for later use
;if (jtf_img eq -1) then begin
;   jtf_img=1
;   f_img='/disks/horae/raid2/themis_gbo/data/imager/rsync_stream0/2006/01/01/'+$
;       'rank_themis01/ut06/20060101_0600_rank_themis01_full_1000ms.pgm.gz'
;   print,'DUMMY file'
;   endif
	   ...
	   if keyword_set(full_minute) then begin
             ...
             for ijk=0,19 do begin
               ...
               u=rotate(u,7)              ;rotation of image to match skymap arrays
               s1[*,*,ijk]=u
               endfor
             u=s1
		...
		; calibration data if there are images
 		...
 		; restore save-files
           ...
           f_skymap=findfile(sky_dir_0+'themis_skymap*'+station_string+$
                '*19700101.sav')
           if keyword_set(verbose) then print,'VERBOSE_COMMENT_02:'+$
               '  '+f_skymap(0)
           restore,f_skymap(0)
