...
pro thm_map_oplot_aacgm_2000_invariants,aacgm_lon_contour,aacgm_lat_contour,$
   ...
   if keyword_set(invariant_lats) then begin
      for i=0,n_elements(invariant_lats)-1 do begin
          j=invariant_lats(i)
          jt=j mod 5
          if jt ne 0 then print,'element '+strcompress(string(i),/remove_all)+' of invariant_lats is not valid'
          if jt eq 0 then oplot,v_lon(j/5-10,*),v_lat(j/5-10,*),color=inc,thick=invariant_thick,noclip=0,linestyle=invariant_linestyle
      endfor
   endif
   if keyword_set(invariant_lons) and not keyword_set(invariant_lats) then print,'you must set invariant_lats if you set invariant_lons
   if keyword_set(invariant_lons) and keyword_set(invariant_lats) then begin
      if n_elements(invariant_lats) eq 1 then print,'note - invariant_lons has no effect if n_elements(invariant_lats)=1
