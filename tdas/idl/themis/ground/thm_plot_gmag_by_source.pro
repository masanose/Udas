...
;________________________________________________________________________________________________
  ...
  If(is_string(source_in)) Then Begin
    ...
    If(is_string(sources) Eq 0) Then Begin
      message, /info, 'No valid sources input'
      Return
    Endif
...
;plot each source
  For j = 0, nsource-1 Do Begin
    ...
    message, /info, 'PROCESSING '+strupcase(thetitle)
    Case sources[j] Of
...
;________________________________________________________________________________________________
    ...
    If(is_string(tplotvars) Eq 0) Then Begin
      message, 'Ok sites:', /info
      print, 'None'
      message, 'Missing sites:', /info
      for k = 0, nj-1 do print, sitesj[k]
      ss_ok = -1
    Endif Else Begin
      ss_ok = sswhere_arr(test_vars, tplotvars)
      message, 'Ok sites:', /info
      for k = 0, n_elements(ss_ok)-1 do print, sitesj[ss_ok[k]]
      ss_miss = sswhere_arr(test_vars, tplotvars, /notequal)
      message, 'Missing sites:', /info
      If(ss_miss[0] Ne -1) Then Begin
        for k = 0, n_elements(ss_miss)-1 do print, sitesj[ss_miss[k]]
