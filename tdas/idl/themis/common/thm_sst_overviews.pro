...
if not keyword_set(date) then begin
    message,/continue,'Date must be set to generate sst overview plots'
    return
endif
...
;    index_sst_e=where(strjoin('th'+sc+'_psef_en_eflux') eq tnames())
;    index_sst_i=where(strjoin('th'+sc+'_psif_en_eflux') eq tnames())
;    if(index_sst_e[0] eq -1 Or index_sst_i[0] Eq -1) then begin
    thm_load_sst, probe = sc, level = 'l1'
;check for data before doing moments
    instr_do = ['psif', 'psef', 'psir', 'pser']
    ss_do = bytarr(n_elements(instr_do))
    For j = 0, n_elements(instr_do)-1 Do Begin
      index_sst = where(strjoin('th'+sc+'_'+instr_do[j]+'_tot') eq tnames())
      If(index_sst[0] Ne -1) Then ss_do[j] = 1b
    Endfor
    ok_instr = where(ss_do Eq 1)
    If(ok_instr[0] Eq -1) Then Begin
      message, /info, 'NO SST Data for Probe: '+sc
    Endif Else Begin
