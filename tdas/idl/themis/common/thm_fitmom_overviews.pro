...
;      if(max2 gt maxf) then maxf = max else minf=max2
   ...
   message,/continue,'Mnemonic does not exist: ' + mnem
endelse
...
pro thm_fitmom_overviews,date,probe,directory=directory,device=device,makepng=makepng
...
if not keyword_set(date) then begin
    message,/continue,'Date must be set to generate fields and moments overview plots'
    return
endif
...
if not keyword_set(probe) then begin
    message,/continue,'probe must be set to generate fields and moments overview plots'
    return
endif
...
if(strfilter(probe_list,probe) eq '') then begin
   message,/continue,'probe must be valid to generate fields and moments overview plots'
    return
endif
...
if(probe eq 'a' && time_double(date) lt time_double('2007-08-02/:00:00')) then begin
  message, /continue, 'on board moments not yet corrected at: ' + date
  return
endif

if(probe eq 'b' && time_double(date) lt time_double('2007-08-10/:00:00')) then begin
  message, /continue, 'on board moments not yet corrected at: ' + date
  return
endif

if(probe eq 'c' && time_double(date) lt time_double('2007-08-10/00:00:00')) then begin
  message, /continue, 'on board moments not yet corrected at: ' + date
  return
endif

if(probe eq 'd' && time_double(date) lt time_double('2007-08-10/00:00:00')) then begin
 message, /continue,'on board moments not yet corrected at: ' + date
  return
endif

if(probe eq 'e' && time_double(date) lt time_double('2007-08-10/00:00:00')) then begin
  message, /continue, 'on board moments not yet corrected at: ' + date
  return
endif
...
if tnames(fgs_name) then begin
   ...
   set_lim,fgs_name+'+t',times[0],times[1],-100D,100D,0
   ...
   blank_panel,fgs_name+'+t','th'+probe+'!Cfgs!Cgsm!C[nT]',labels=['Bx','By','Bz','Bt']
...
;ensure that the y scale is bounded at +- 100 nT
   ...
   set_lim,fgs_name+'-t89',times[0],times[1],-100D,100D,0

   options,fgs_name+'-t89',ytitle='th'+probe+'!Cfgs!Cgsm!C-t89',ysubtitle='[nT]',labels=['Bx','By','Bz']

endif else begin
  
 blank_panel,fgs_name+'-t89','th'+probe+'!Cfgs!Cgsm!C-t89!C[nT]',labels=['Bx','By','Bz']
...
thm_scpot2dens_opt_n, probe = probe, /no_data_load, datatype_esa = 'peef'
copy_data, 'th'+probe+'_peef_density_npot', 'Npot'
...
if tnames('Npot') && tnames('th'+probe+'_peem_density') then begin

   get_data,'th'+probe+'_peem_density',data=d
   ...
   store_data,'den_mom',data=['Npot','th'+probe+'_peim_density','dlab_kluge']
   
endif else if tnames('th'+probe+'_peem_density') then begin 
   ...
   store_data,'den_mom',data=['th'+probe+'_peim_density','dlab_kluge']

endif

options,'th'+probe+'_peim_density',colors=0

;options,'th'+probe+'_peer_density',colors=6,labels='Ne',labflag=2

if tnames('den_mom') then begin

options,'den_mom','ytitle','Density'

set_lim,'den_mom',times[0],times[1],0,200,1

endif else begin
   
   blank_panel,'den_mom','Density!C[1/cc]',labels=['Npot','Ni','Ne']
...
if tnames('th'+probe+'_peir_magt3') && $
   ...
   set_lim,'Tieperpara',times[0],times[1],-1*!VALUES.D_INFINITY,!VALUES.D_INFINITY,1

endif else begin

   blank_panel,'Tieperpara','magt3!C[eV]',labels=['ti_para','ti_perp','te_para','te_perp']
...
if tnames(pei_vel_name) then begin
   ...
   thm_cotrans,pei_vel_name,in_c='dsl',out_c='gsm'

   options,pei_vel_name,'labflag',1,def=1

   tvectot,pei_vel_name,newname=pei_vel_name+'+t'

   options,pei_vel_name+'+t',ytitle='th'+probe+'!Cpeim!Cvelocity!Cgsm'
   ...
   set_lim,pei_vel_name+'+t',times[0],times[1],-1500D,1500D,0

endif else begin

   blank_panel,pei_vel_name+'+t','th'+probe+'!Cpeir!Cvelocity!C[km/s]',labels=['Vx','Vy','Vz','Vt']
...
pee_vel_name = 'th'+probe+'_peem_velocity'

if tnames(pee_vel_name) then begin
   
   thm_cotrans,pee_vel_name,in_c='dsl',out_c='gsm'
   
   options,pee_vel_name,'labflag',1,def=1

   tvectot,pee_vel_name,newname=pee_vel_name+'+t'

   options,pee_vel_name+'+t',ytitle='th'+probe+'!Cpeem!Cvelocity!Cgsm'
   ...
   set_lim,pee_vel_name+'+t',times[0],times[1],-1500D,1500D,0

endif else begin

   blank_panel,pee_vel_name+'+t','th'+probe+'!Cpeim!Cvelocity!C[km/s]',labels=['Vx','Vy','Vz','Vt']
...
if tnames('th'+probe+'_peim_velocity_gsm') && $
   ...
   endif else  blank_panel,'e_eq_nvxb','E=-VxB!C[mV/m]',labels=['Ex','Ey','Ez']

endif else begin

   blank_panel,'e_eq_nvxb','E=-VxB!C[mV/m]',labels=['Ex','Ey','Ez']
...
;      I attach some code to see how I did it before and check.
...
if tnames('th'+probe+'_fgs_gsm') && $
   ...
   get_data,'th'+probe+'_peem_ptens',data=d_peem

   Pb = total(d_fgs.y^2,2) * (1D/2513.2741D)

   Pi = (d_peim.y[*,0]+d_peim.y[*,1]+d_peim.y[*,2])/3D * 1.6e-4

   Pe = (d_peem.y[*,0]+d_peem.y[*,1]+d_peem.y[*,2])/3D * 1.6e-4

   Pb_i = interpol(pb,d_fgs.x,d_peim.x)

   Pe_i = interpol(pe,d_peem.x,d_peim.x)

   Pt = Pi + Pb_i + Pe_i

   d = {x:d_peim.x,y:[[Pi],[Pe_i],[Pb_i],[Pt]]}
   ...
   blank_panel,'pressure_vars','Pressure!C[nPa]',labels=['Pi','Pe','Pb','Pt']
...
if tnames('th'+probe+'_peir_en_eflux') then begin
   ...
   thm_spec_lim4overplot,'th'+probe+'_peir_en_eflux',zlog=1,ylog=1,/overwrite
   ...
   blank_panel,'th'+probe+'_peir_en_eflux','thb!Cpeir!Cen_eflux!CeV/!C(cm^2-s-!Csr-eV)'
...
if tnames('th'+probe+'_peer_en_eflux') then begin
   ...
   thm_spec_lim4overplot,'th'+probe+'_peer_en_eflux',zlog=1,ylog=1,/overwrite
   ...
   blank_panel,'th'+probe+'_peer_en_eflux','thb!Cpeer!Cen_eflux!CeV/!C(cm^2-s-!Csr-eV)'
...
set_lim,'Tieperpara',times[0],times[1],-1*!VALUES.D_INFINITY,!VALUES.D_INFINITY,1
set_lim,pei_vel_name+'+t',times[0],times[1],-1500D,1500D

set_lim,pee_vel_name+'+t',times[0],times[1],-1500D,1500D

set_lim,'pressure_vars',times[0],times[1],0D,50D,1

;Bottom tick marks:

;Show UT, and GSM X, Y, Z (same as the overview plots, except in GSM).

if tnames('th'+probe+'_state_pos') then begin

   get_data,strjoin('th'+probe+'_state_pos'),data=tmp

   store_data,strjoin('th'+probe+'_state_pos_gsm_x'),data={x:tmp.x,y:tmp.y[*,0]/6374.}
   options,strjoin('th'+probe+'_state_pos_gsm_x'),'ytitle','th'+probe+'_X-GSM'
  store_data,strjoin('th'+probe+'_state_pos_gsm_y'),data={x:tmp.x,y:tmp.y[*,1]/6374.}
  options,strjoin('th'+probe+'_state_pos_gsm_y'),'ytitle','th'+probe+'_Y-GSM'
  store_data,strjoin('th'+probe+'_state_pos_gsm_z'),data={x:tmp.x,y:tmp.y[*,2]/6374.}
...
title =  probes_title[pindex[0]]+' (TH-'+strupcase(scv)+') fields and on-board moments overview'
...
if keyword_set(makepng) then begin
  ...
  ymd = year+month+day
  if keyword_set(directory) then dir=directory else dir='./'
  makepng,dir+'th'+probe+'_l2_moms_'+year+month+day+'_0024',/no_expose
;six-hour plots
  For j = 0, 3 Do Begin
    hrs0 = 6*j
    hrs1 = 6*j+6
...
;Set limits for this time period
    set_lim, fgs_name+'+t', tr0[0], tr0[1], -100D, 100D, 0
    set_lim, fgs_name+'-t89', tr0[0], tr0[1], -100D, 100D, 0
    set_lim, 'den_mom', tr0[0], tr0[1], 0, 200, 1
    set_lim, 'Tieperpara', tr0[0], tr0[1], -1*!VALUES.D_INFINITY, !VALUES.D_INFINITY, 1
    set_lim, pei_vel_name+'+t', tr0[0], tr0[1], -1500D, 1500D
    set_lim, pee_vel_name+'+t', tr0[0], tr0[1], -1500D, 1500D
    set_lim, 'pressure_vars', tr0[0], tr0[1], 0D, 50D, 1
    tplot, trange = tr0
