...
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/thmsoc/tags/tdas_5_21/idl/themis/state/thm_sunpulse.pro $
;-
; ----------------------------------------------------------------------
  ...
  if n_params() eq 0 then begin
     ...
     for i = 0, n_elements(prb)-1 do begin
        ...
        if (tph ne 1 || tpe ne 1) then begin
           print, '*** thm_sunpulse: state data not loaded for probe ', prb[i]
           continue
        endif
  ...
  PRINT, '   Dimension of state file array = ', nsta
  FLUSH, 0, -1, -2
...
; get number of spins between each known sunpulse time
  ...
  if n_spin_error gt 0 then begin
     print, '*** thm_sunpulse: spin rate changing too quickly to uniquely'
     print, '    determine sunpulses between state data points at ', $
            time_string(time_state[spin_error[0:10 < n_spin_error-1]])
     if n_spin_error gt 10 then $
        print, '    ', n_spin_error, ' times. Output truncated'
  endif
  ...
  if n_spin_warn gt 0 then begin
     print, '*** thm_sunpulse: spin rate data questionable at ', $
            time_string(time_state[spin_warn[0:10 < n_spin_warn-1]])
     if n_spin_warn gt 10 then $
        print, '    ', n_spin_warn, ' times. Output truncated'
  endif
