...
;
; $LastChangedBy: jwl $
; $LastChangedDate: 2010-02-05 15:45:45 -0800 (Fri, 05 Feb 2010) $
; $LastChangedRevision: 7219 $
; $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/thmsoc/tags/tdas_5_21/idl/themis/state/cotrans/thm_fac_matrix_make.pro $
;-
...
if not keyword_set(mag_var_name) then begin
    message,/continue,' fx requires mag_var_name to be set'
...
if(co_idx[0] eq -1L) then begin
    message,/continue,' fx was passed an illegal output coordinate'+other_dim
    message,/continue,' fx requires output other_dim to be: ' + strjoin(valid_coords,',')
...
if error_state eq 0 then begin
    message,/continue,' fx failed to normalize magnetic field vector'
...
if(n_elements(d_s) ne 2 || d_s[1] ne 3) then begin
       message,/continue,' fx requires mag_var_name data to be an Nx3 Array'
       return
endif
...
if keyword_set(pos_var_name) then begin
  if tnames(pos_var_name) eq '' then begin
     message,/continue,'illegal position tplot variable name'
     return
  endif

  if n_elements(tnames(pos_var_name)) ne 1 then begin
     message,/continue,'can only input a single position tplot variable'
     return
  endif

  if cotrans_get_coord(pos_var_name) ne 'gei' then begin
     message,/continue,'position tplot variable must be in gei coordinates'
     return
  endif
...
case other_dim_l of
  ...
  valid_coords[0] : begin
    ...
    if(dl.data_att.coord_sys ne 'gse' && $
       ...
       dl.data_att.coord_sys ne 'dsl') then begin
        ...
        message,/continue,' fx requires mag_var_name to be in GSE, GSM, or DSL to generate a Xgse field aligned coordinate matrix'
        return
    endif
    ...
    if(error_state eq 0) then begin
         message,/continue,' FX  failed to create zx cross product in ' + valid_coords[0]
        return
    endif
    ...
    if(error_state eq 0) then begin
         message,/continue,' FX  failed to create zx cross product in Xgse'
        return
    endif
    ...
    if(error_state eq 0) then begin
        message,/continue,' FX  failed to create yz cross product in Xgse'
        return
    endif
  ...
  valid_coords[1] : begin
    ...
    if not keyword_set(pos_var_name) then begin
        message,/continue,' FX  requires pos_var_name to be set for Rgeo coordinate transformation'
        return
    endif
    ...
    if(n_elements(d_2_s) ne 2 || d_2_s[1] ne 3) then begin
        message,/continue,' FX  requires pos_var_name data to be an Nx3 Array'
        return
    endif
    ...
    if dl_2.data_att.coord_sys ne 'gei' then begin
      ...
      if dl_2.data_att.coord_sys eq 'dsl' then begin
        ...
        message,/continue,' FX  requires position data to be in gei or dsl coordinates to generate Rgeo transformation'
        return
      endelse
    ...
    if error_state eq 0 then begin
        message,/continue,' FX  failed to interpolate position data onto mag field data'
        return
    endif
    ...
    if error_state eq 0 then begin
        message,/continue,' FX  failed to normalize interpolated position data'
        return
    endif
    ...
    case dl.data_att.coord_sys of
     ...
     'dsl' : begin
               probe = strmid(pos_var_name_temp,2,1)
               thm_cotrans,'fac_mat_pos_temp','fac_mat_pos_temp',in_coord='gei',out_coord='dsl',$
                           probe=probe
             end
      else : begin
               message,/continue,' FX  failed to rotate data onto mag field system'
               return
             end
    endcase
    ...
    if(error_state eq 0) then begin
        message,/continue,' FX  failed to generate B x Rgeo vector'
        return
    endif
    ...
    if error_state eq 0 then begin
      message,/continue,' FX  failed to normalize y-vector after cross-product'
      return
    endif  
    ...
    if(error_state eq 0) then begin
        message,/continue,' FX  failed to generate Y x Z vector'
        return
    endif
  ...
  valid_coords[2] : begin
    ...
    if not keyword_set(pos_var_name) then begin
        message,/continue,' FX  requires pos_var_name to be set for mRgeo coordinate transformation'
        return
    endif
    ...
    if dl_2.data_att.coord_sys ne 'gei' then begin
      ...
      if dl_2.data_att.coord_sys eq 'dsl' then begin
        ...
        message,/continue,' FX  requires position data to be in gei or dsl coordinates to generate mRgeo transformation'
        return
      endelse
    ...
    if(error_state eq 0) then begin
        message,/continue,' FX  failed to generate Rgeo x B = B x mRgeo vector'
        return
    endif
  ...
  valid_coords[3] : begin
    ...
    if not keyword_set(pos_var_name) then begin
        message,/continue,' FX requires pos_var_name to be set for Phigeo coordinate transformation'
        return
    endif
    ...
    if dl_2.data_att.coord_sys ne 'gei' then begin
      ...
      if dl_2.data_att.coord_sys eq 'dsl' then begin
        ...
        message,/continue,' FX  requires position data to be in gei or dsl coordinates to generate Phigeo transformation'
        return
      endelse
    ...
    if(error_state eq 0) then begin
        message,/continue,'FX failed to calculate Phigeo by z cross product'
        return
    endif
    ...
    if(error_state eq 0) then begin
        message,/continue,'FX failed to normalize x temp'
        return
    endif
    ...
    if(error_state eq 0) then begin
        message,/continue,'FX failed to calculate z by x cross product'
        return
    endif
  ...
  valid_coords[4] : begin
    ...
    if not keyword_set(pos_var_name) then begin
        message,/continue,' FX  requires pos_var_name to be set for mPhigeo coordinate transformation'
        return
    endif
    ...
    if dl_2.data_att.coord_sys ne 'gei' then begin
      ...
      if dl_2.data_att.coord_sys eq 'dsl' then begin
        ...
        message,/continue,' FX  requires position data to be in gei or dsl coordinates to generate Rpos transformation'
        return
      endelse
    ...
    if(error_state eq 0) then begin
        message,/continue,'FX failed to calculate mPhigeo by z cross product'
        return
    endif
  ...
  valid_coords[5]: begin
    ...
    if not keyword_set(pos_var_name) then begin
        message,/continue,' FX  requires pos_var_name to be set for Phism coordinate transformation'
        return
    endif
    ...
    if dl_2.data_att.coord_sys ne 'gei' then begin
      ...
      if dl_2.data_att.coord_sys eq 'dsl' then begin
        ...
        message,/continue,' FX  requires position data to be in gei or dsl coordinates to generate Phism transformation'
        return
      endelse
    ...
    if(error_state eq 0) then begin
        message,/continue,'FX failed to calculate pos by z cross product'
        return
    endif
  ...
  valid_coords[6]: begin
    ...
    if not keyword_set(pos_var_name) then begin
        message,/continue,' FX  requires pos_var_name to be set for mPhism coordinate transformation'
        return
    endif
    ...
    if(error_state eq 0) then begin
        message,/continue,'FX failed to calculate mPhism by z cross product'
        return
    endif
  ...
  valid_coords[7]: begin
    ...
    if(dl.data_att.coord_sys ne 'gse' && $
       dl.data_att.coord_sys ne 'gsm' && $
       dl.data_att.coord_sys ne 'dsl') then begin
        message,/continue,' fx requires mag_var_name to be in GSE, GSM, or DSL to generate a Ygsm field aligned coordinate matrix'
        return
    endif
    ...
    case dl.data_att.coord_sys of
     ...
     'dsl' : begin
               ...
               message,/continue,' FX failed to rotate Ygsm vector onto input mag field system'
               return
             end
    endcase
    ...
    if(error_state eq 0) then begin
         message,/continue,' FX  failed to create yz cross product in Ygsm'
        return
    endif
    ...
    if(error_state eq 0) then begin
         message,/continue,' FX  failed to normalize Ygsm in Ygsm'
        return
    endif
    ...
    if(error_state eq 0) then begin
        message,/continue,' FX  failed to create zx cross product in Ygsm'
        return
    endif
...
;    endcase
    ...
    if s1 eq 1 then $
      ...
      message,/continue,'no valid other_dimension'
               return
  end
endcase
