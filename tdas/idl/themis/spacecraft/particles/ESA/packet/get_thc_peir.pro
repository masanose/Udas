...
;-
FUNCTION get_thc_peir,time,START=st,EN=en,ADVANCE=adv,RETREAT=ret,index=ind,calib=calib,times=times

common thc_455,get_ind,all_dat

if n_elements(get_ind) eq 0 then begin
	dat = 	{project_name:'THEMIS',valid:0}
	print,' ERROR - thc peir data not loaded'
...
;	print,nbins,nenergy,an_ind,en_ind,mode,config1,config2
	...
	case all_dat.cs_ind[ind] of
		...
		4: dat=transpose(reform(decompress(all_dat.dat4[cs_ptr,*]),nbins,nenergy))
		5: dat=transpose(reform(decompress(all_dat.dat5[cs_ptr,*]),nbins,nenergy))
	endcase
