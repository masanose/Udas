...
pro thm_load_peef,file=file,sc=sc,themishome=themishome,gap_time=gap_time,suffix=suffix
...
; define mode variables for different modes
	nspins = [3,128,32,128,32,1]				; # of spins between measurements in mode
	nenergy = [32,32,32,32,32,15]				; # of energies in mode
	nangle = [88,88,88,88,88,88]				; # of angles in mode
	dat_len = nenergy*nangle				; size data arrays
	spin_decode  = [1,1,1,1,1,3]				; # measurements in packet
	case_decode  = [0,0,0,0,0,1]				; datl[2861,2861,2861,2861,2861,1320]==>size[0,0,0,0,0,1]
	angle_decode = [0,0,0,0,0,1]				; angle mode index
	energy_decode = [0,1,1,2,2,3]				; energy mode index
	
; initialize arrays
	ndays=n_elements(file)
	nmax=40000l*ndays
	dat_0=bytarr(nmax/4<40000l,2816) 
	dat_1=bytarr(nmax/2<40000l,1320) 
...
;			printdat,fs
	...
	print,'457 Download time= ',tm2-tm1
...
;	help,adat
	print,'Decommutating data ...  Data volume (bytes) =',data_size
...
endwhile
	...
	tm2=systime(1)
	print,'457 Decommutation time= ',tm2-tm1
...
corr1=1.
;  lf=4.
;  corr=1.0*(1.+.32*(1.-alog(lf*energy)/8.9)*exp(-lf*energy/350.) + .05*(1.-alog(lf*energy)/12.))	;due to leakage fields into ESA
;  corr1=   (1.+.10*((1.-alog(energy/.05)/7.8)>0.))							;due to ESA exit grid 
...
; help,efd_dat,/st
print,'Data loading complete: th'+sc+' peef - apid 457'
