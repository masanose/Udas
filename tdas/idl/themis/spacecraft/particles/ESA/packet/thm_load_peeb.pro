...
pro thm_load_peeb,file=file,sc=sc,themishome=themishome,gap_time=gap_time,suffix=suffix
...
; sc default
	if not keyword_set(sc) then begin
		print,'S/C number not set, default = a'
		sc='a'
	endif
...
; check that files exist
	...
	if nfiles eq 1 then begin
		...
		for i=0,nfiles-1 do begin
			if file_test(file[i]) then ind=[ind,i] else print,file[i]+' --- does not exist.'
		endfor
...
; define mode variables for different modes
	nspins = [1,1,1,1,1,1]				; # of spins between measurements in mode
	nenergy = [32,32,32,32,32,32]				; # of energies in mode
	nangle = [88,88,88,88,88,88]				; # of angles in mode
	dat_len = nenergy*nangle				; size data arrays
	spin_decode  = [1,1,1,1,1,1]				; # measurements in packet
	case_decode  = [0,0,0,0,0,0]				; datl[16,32,96,192,1152,1200]==>size[0,1,2,3,4,5]
	angle_decode = [0,0,0,0,0,0]				; angle mode index
	energy_decode = [0,1,1,2,2,1]				; energy mode index
...
; get the files
	...
	print,'459 Download time= ',tm2-tm1
...
while (tf_spin and i lt 100 and pntr+hdrl+pktl lt data_size and data_size gt 4000) do begin
	i=i+1
	dprint,dlevel=1,'tf_spin,i=',tf_spin,i
	...
	dprint,dlevel=1,'Config bytes = ',config
	...
	dprint,dlevel=1,pntr,'  ',pktl,'  ',time_string(time),'  ',adat[pntr+hdrl],'  ',adat[pntr+hdrl+1],'  spin period= ',spin_period
endwhile
	...
	dprint,dlevel=1,'spin_period = ',spin_period
...
;	help,adat
	dprint,dlevel=1,'Decommutating data ...  Data volume (bytes) =',data_size
	dprint,dlevel=1,'Current data	pktl	time				config bytes '
	...
	if spindata then begin
		if n_elements(spinmodel.s_time) eq 1 then begin
			...
			if abs(time2-time+offset) gt .15 then dprint,dlevel=1,'ERROR: peeb Header time differs from spin model spin-start time by: '$
				,time2-time,-1.*offset,apid_cntr,last_apid_cntr,' th'+sc,' ',time_string(last_time+t0),' '$
				,time_string(time+t0),' ',spinmodel.spin_period,' ',pntr
			dprint,dlevel=1,spinmodel.s_time,time2,time,time2-time
		endif else begin
			...
			if abs(time2-time+offset) gt .15 then dprint,dlevel=1,'ERROR: peeb Header time differs from spin model spin-start time by: '$
				,time2-time,-1.*offset,apid_cntr,last_apid_cntr,' th'+sc,' ',time_string(last_time+t0),' '$
				,time_string(time+t0),' ',spinmodel.spin_period(ind),' ',pntr
			dprint,dlevel=1,spinmodel.s_time(ind),time2,time,time2-time
		endelse
	endif
	pktl = hdr[4]*256+hdr[5]+7-hdrl
	pktp = 0l & datl=0
...
endwhile
	...
	tm2=systime(1)
	print,'459 Decommutation time= ',tm2-tm1
...
; help,ebd_dat,/st
print,'Data loading complete: th'+sc+' peeb - apid 459'
...
for i=0l,ntot-1l do begin
	dprint,dlevel=1,ntot,i,cs_ptr[i],cs_ind[i]
	case cs_ind[i] of
