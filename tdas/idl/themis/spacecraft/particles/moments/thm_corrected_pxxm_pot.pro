...
Function tmp_load_mom_calfile, probe, version
  ...
  Endif Else Begin
    message, /info, 'Version 2 cal files not found for probe: '+pj+'. No corrections are possible.'
    caldata = -1
  Endelse
  Return, caldata
End
...
;$LastChangedBy: jimm $
;$LastChangedDate: 2010-02-09 15:37:45 -0800 (Tue, 09 Feb 2010) $
;$LastChangedRevision: 7258 $ 
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/thmsoc/tags/tdas_5_21/idl/themis/spacecraft/particles/moments/thm_corrected_pxxm_pot.pro $
;-
...
;first check for 'pxxm_pot' variables
  ...
  If(is_string(xvarx) Eq 0) Then Begin
    message, /info, 'No PXXM_POT variables loaded'
    Return
  Endif
...
;corrected=1, if found, that ones doesn't need correcting
  ...
  If(keep[0] Ne -1) Then xvarx = xvarx[keep] Else Begin
    message, /info, 'No uncorrected PXXM_POT variables available'
    Return
  Endelse
  ...
  For j = 0, nx-1 Do Begin
    ...
    If(is_struct(d)) Then Begin
      ...
      If(is_struct(caldata1) Eq 0 Or is_struct(caldata2) Eq 0) Then Begin
        message, /info, 'No calibration data for probe: '+pj
        Continue
      Endif
...
;Need the spin period
        ...
        If(is_struct(spinper) Eq 0) Then Begin ;load the state data if we don't have it
          ...
          If(is_struct(spinper) Eq 0) Then Begin
            message, /info, 'No state data available for probe: '+pj[0]
            message, /info, 'Using default 3 sec spin period'
        ...
        If(d.x[0] Ge mom_tim_adjust[pjno]) Then Begin
          time = d.x-tshft_mom[1]*spin_period
        Endif Else If(d.x[npts-1] Lt mom_tim_adjust[pjno]) Then Begin
          time = d.x-tshft_mom[0]*spin_period
        Endif Else Begin
