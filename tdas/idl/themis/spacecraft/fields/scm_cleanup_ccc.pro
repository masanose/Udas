...
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/thmsoc/tags/tdas_5_21/idl/themis/spacecraft/fields/scm_cleanup_ccc.pro $
;-
...
function scm_cleanup_ccc,noisy_data,$
	...
	print,'start time ',time_string(datax.x(0), precision=7)
	print,'stop time ', time_string(datax.x(number_points-1), precision=7)
	...
	print,index_range(0),index_range(n_elements(index_range)-1)
	...
	print,'mininum number of windows asked',minimum_windows
	print,'available number of windows',n_stretch
	...
	if n_stretch GE minimum_windows then begin
		...
		endif else print,'insufficient continuous data for this interval'
