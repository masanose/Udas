...
;$LastChangedDate: 2009-12-15 13:22:40 -0800 (Tue, 15 Dec 2009) $
;$LastChangedRevision: 7004 $
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/thmsoc/tags/tdas_5_21/idl/themis/spacecraft/fields/thm_cal_fgm_spin_harmonics.pro $
;-
...
pro thm_cal_fgm_spin_harmonics,times,spinphase,fgmdata,probe,error=error,shadows=shadow_struct
  ...
  dprint,'Using spin harmonic correction.'
  ...
  for i = ts[0].year,ts[1].year do begin
    ...
    for j = startmonth,endmonth do begin
      ...
      if keyword_set(shadow_struct) && is_struct(shadow_struct) then begin
      ...
      endif else if tnames('th'+probe+'_state_roi*') then begin
        ...
        if c gt 0 then begin
          dprint,'Using: ' + roi_name[0] + ' to determine when spacecraft is in shadow'
          ...
          dprint,'WARNING: ' + roi_name[0] + ' does not overlap with state data.  Spin Harmonic Correction may be applied during shadow'
        endelse
      
      endif else begin
      
        dprint,'WARNING: th?_state_roi not present, spin harmonic correction will be applied during shadow.' 
      ...
      if ~file_test(month_file,/read) then begin
        dprint,'WARNING: Could not find spin harmonic calibration for month starting at: ' + $
               num_to_str_pad(stime_struct.year,4)+'/'+$
               num_to_str_pad(stime_struct.month,2)+'/01, no readable spin_cal file.'

        dprint,'Searching for Default Calibration File...'
      ...
      dprint,'Using spin harmonic offsets from calibration file: ' + month_file  
