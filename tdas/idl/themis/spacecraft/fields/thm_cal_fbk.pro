...
; $LastChangedBy: jwl $
; $LastChangedDate: 2010-01-25 12:16:07 -0800 (Mon, 25 Jan 2010) $
...
if arg_present( valid_names) then begin
	...
	message, /info, string( strjoin( fbk_valid_names, ','), format='( "Valid names:",X,A,".")')
	return
endif
...
if not keyword_set(probe) then probe = vprobes $
  ...
  if keyword_set(verbose) then print, probe
