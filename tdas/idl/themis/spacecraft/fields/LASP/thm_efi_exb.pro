...
IF keyword_set(Sname) THEN BEGIN
  ...
  S.y[*,2] = u0*Sz*1e3
  ; STORE
  data_att = {DATA_TYPE: 'calibrated', COORD_SYS: 'gsm', UNITS: 'W/m^2 x 10^-15'}
...
IF keyword_set(Vname) THEN BEGIN
  ...
  data_att = {DATA_TYPE: 'calibrated', COORD_SYS: 'gsm', UNITS: 'km/s'}
  dlim = {SPEC: 0b, LOG: 0b, COLORS: elim.colors, $
    YSUBTITLE: '(km/s)', YTITLE: 'ExB Velocity', $
    LABELS: elim.labels, LABFLAG: 1L, DATA_ATT: data_att}
