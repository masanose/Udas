...
;
  ...
  if non_monoton_detected  then begin
    message, /info, '*** WARNING: Non-monotonic time tags detected.  Sorting data'
    ss_sorted = bsort(arr_in_t)
...
;  beta = A parameter for finding fits.  See above.  Default = 0.4
; $LastChangedBy: jimm $
; $LastChangedDate: 2010-03-24 15:51:51 -0700 (Wed, 24 Mar 2010) $
; $LastChangedRevision: 7437 $
; $URL:
;-
...
pro thm_spinavg, var_name_in, sigma = sigma, npoints = npoints, $
  ...
  If(is_string(vn) Eq 0) Then Begin
    message, /info, 'No variable: '+var_name_in
    Return
  Endif
  ...
  For i = 0, nvn-1 Do Begin
    ...
    Endif
    If(is_struct(thx_spinper) Eq 0) Then Begin
      message, /info, 'No Spin Period Available: '+vn[i]
      Continue
    Endif
