...
;
; $LastChangedBy: jimm $
; $LastChangedDate: 2010-03-24 16:49:49 -0700 (Wed, 24 Mar 2010) $
; $LastChangedRevision: 7438 $
; $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/thmsoc/tags/tdas_5_21/idl/themis/spacecraft/fields/thm_load_efi_l2.pro $
;-
...
; If verbose keyword is defined, override !themis.verbose
  ...
  thm_load_proc_arg, sname = probe, datatype = datatype, $
    ...
    vsnames = 'a b c d e', $
    type_sname = 'probe', $
    vdatatypes = 'eff_dot0 efs_dot0 eff_e12_spinav eff_e34_spinav eff_data_quality', $
    vtypes = 'calibrated', $
    vlevels = 'l2', $
...
;; my_themis is a copy of !themis, which may have no_download set
    ...
    if keyword_set(trange) then begin
      for l = 0, n_elements(tplotnames)-1 do begin
        ...
        print, 'THM_LOAD_EFI_l2: Clipping '+tplotnames[l]
        time_clip, tplotnames[l], trange[0], trange[1], /replace, error = clip_err
        if clip_err then begin
          print, 'THM_LOAD_EFI_l2: Unable to clip '+tplotnames[l]+' to requested time range. Data may be out of range.'
...
;; add DLIMIT tags to data quantities, and suffix if necessary
    for l = 0, n_elements(tplotnames)-1 do begin
      ...
      if is_struct(dl_str) && dl_str.cdf.vatt.var_type eq 'data' then begin
...
;'unit' variable, since that goes to ysubtitle
      ...
      endif else if is_struct(dl_str) && dl_str.cdf.vatt.var_type eq 'support_data' then begin
        thm_new_units, tplot_var
        get_data, tplot_var, dlimits = dl_str
        unit = dl_str.data_att.units
        u1 = strsplit(unit, ' ', /extract)
        dl_str.data_att.units = u1[0]
        If(strpos(tplot_var, 'e12') Gt 0) Then Begin
          colors = 2
          labels = ['E12 spinavg']
        Endif Else If(strpos(tplot_var, 'e34') Gt 0) Then Begin
          colors = 4
          labels = ['E34 spinavg']
        Endif Else Begin
          colors = 0
