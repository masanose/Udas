...
if non_monoton_detected  then begin
  message, /info, '*** WARNING: Non-monotonic time tags detected.  1 s chunks of data discarded until time tags are monotonic, but consult '+ $
...
i=0L
j = (where(arr_in_t ge arr_in_sunpulse_t[overlap1[i]] and arr_in_t le arr_in_sunpulse_t[overlap1[i+1]]))[0]
for i=0L, sizeoverlap[1]-2 do begin

  ; select a one period chunk of data:
  ;
  overlap = j
  while ( j lt size_xxx[1] ) && ( arr_in_t[j] le arr_in_sunpulse_t[overlap1[i+1]] ) do begin
    overlap = [ overlap, j ]
    ++j
  endwhile
