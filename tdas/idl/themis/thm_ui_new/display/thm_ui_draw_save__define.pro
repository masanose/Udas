...
function thm_ui_draw_save::write,fileinfo
...
;      dim = olddim*oldres/res
  ...
  ;  currentWin->getProperty,image_data=img
    ...
    self.drawObject->draw,vector=vector,postscript=ps,filename=file
    ...
    if format eq '.png' then begin
     ...
     endif else if format eq '.ppm' then begin
      ...
      ;This code performs top-bottom image inversion
      img[0,*,*] = img[0,*,reverse(indgen(dim[2]))]
      img[1,*,*] = img[1,*,reverse(indgen(dim[2]))]
      img[2,*,*] = img[2,*,reverse(indgen(dim[2]))]
      
      write_ppm,file,img,ascii=ascii
       
     endif else if format eq '.srf' then begin
...
function thm_ui_draw_save::init,draw,loaded,windows

  compile_opt idl2

  if ~keyword_set(draw) || $
     ~obj_valid(draw) || $
     ~obj_isa(draw,'thm_ui_draw_object') then return,0
     
  if ~keyword_set(loaded) || $
     ~obj_valid(loaded) || $
     ~obj_isa(loaded,'thm_ui_loaded_data') then return,0 

  if ~keyword_set(windows) || $
     ~obj_valid(windows) || $
     ~obj_isa(windows,'thm_ui_windows') then return,0 
...
pro thm_ui_draw_save__define

  struct = { thm_ui_draw_save, $
