...
PRO thm_ui_calendar_update, wbase, state
...
PRO thm_ui_calendar_event, event
  ...
  IF Size(uval, /Type) NE 0 THEN BEGIN
    CASE uval OF
      ...
      'YEAR':BEGIN
               ...
               thm_ui_calendar_update, state.wBase, state
             END
...
PRO thm_ui_calendar, title, otime, gui_id
...
;Create base widgets to hold labels for the selected month, day, and year. Set the initial values of the labels.
   ...
   wHour=thm_ui_spinner(wSubTimeBase, Label='Hour: ', xlabelsize=xls, Increment=1, Value=intime.hour, uval='HOURS')
   wMinute=thm_ui_spinner(wSubTimeBase, Label='Minute: ', xlabelsize=xls, Increment=1, Value=intime.min, uval='MINUTES')
   wSecond=thm_ui_spinner(wSubTimeBase, Label='Second: ', xlabelsize=xls, Increment=1, Value=intime.sec, uval='SECONDS')
...
;List of years created by adding the starting year to an integer array
  years = strtrim((indgen(16)+2000),2)
...
;Create 5 rows of 7 buttons for calendar days 
  ...
  for j=0, n_elements(rows)-1 do begin
    for i=0, 6 do bID = widget_button(rows[j], value='', /no_release, xsize=40, ysize=35, $
                                      uvalue='DAY', uname=strtrim((7*j)+(i),2))
  endfor
  
 
;Create the ok and cancel buttons
   buttonBase = WIDGET_BASE(wBase, /row, /align_center)
   okButton = WIDGET_BUTTON(buttonBase, value='OK', uValue='OK', xsize=75)
...
;Set widget values
   ...
   WIDGET_CONTROL, wYear, SET_VALUE=year 
