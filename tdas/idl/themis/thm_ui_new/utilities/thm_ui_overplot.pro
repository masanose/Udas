...
pro thm_ui_overplot_data_clean,tn_before,dont_delete_data=dont_delete_data
...
If(is_string(string_in) Eq 0) Then Return
...
pro thm_ui_update_dlimits,varname,observatory,instrument,coord_sys,units
  ...
  if is_string(d[0]) then begin
    ...
    for i = 0,n_elements(d)-1 do begin
      thm_ui_update_dlimits,d[i],observatory,instrument,coord_sys,units
    endfor
    return
  endif
  ...
  if ~s then begin
    str_element,dl,'data_att',/add, {PROJECT:'THEMIS',$
    ...
    str_element,data_att,'PROJECT',/add,'THEMIS'
...
pro thm_ui_overplot, windowStorage,loadedData,drawObject,$
...
if ~keyword_set(date) then begin
  ...
  if(t1 Lt time_double('2007-02-17/00:00:00')) then begin
    print, 'Invalid time entered: ', time_string(date)
...
;  options,'thg_pseudoAE',ytitle='THEMIS AE Index'
;Endif Else Begin
;  filler=fltarr(2)
;  filler[*]=float('NaN')
...
;  options,'thg_pseudoAE',ytitle='THEMIS AE Index'
;Endelse
;thm_ui_update_dlimits,'thg_pseudoAE',sc,'ae_index','none','none'
...
options,'thg_idx_ae',ytitle='THEMIS AE Index'
thm_ui_update_dlimits,'thg_idx_ae',sc,'ae_index','none','none'
...
thm_ui_overplot_data_clean,tn_before,dont_delete_data=dont_delete_data
SKIP_GMAG_LOAD:
...
thm_ui_update_dlimits,roi_bar,sc,'roi','none','none'
...
thm_ui_update_dlimits,'Keogram',keo_site,'ask','none','counts'
...
name = thx+'_fgs_gse'
options, name, 'ytitle', 'B FIT GSE (nT)'
...
thm_ui_update_dlimits,'sample_rate_'+sc,sc,'sample_rate','none','none'
...
;kluge to prevent missing data from crashing things
  ...
  if size(dd,/type) ne 8 then begin
    ...
    options, fbk_tvars[i], 'ytitle', thx+' FBK '+strmid(fbk_tvars[i], 7)
...
if ~keyword_set(no_draw) then begin
  ...
  ;set labels
  quick_set_panel_labels, panel[0], ['THEMIS', 'AE Index']
  ...
  ;this code will break if themis color table is not loaded.
  ...
  quick_set_panel_labels, panel[2], '[counts]', /zaxis
  panel[2] -> getproperty, zaxis = zobj
  if(obj_valid(zobj)) then zobj -> setproperty, scaling = 0
  ;Reset labels for FGS data:
  quick_set_panel_labels, panel[3], ['FGS Bx', 'FGS By', 'FGS Bz', '[nT]'], $
    colors_in = [[0b, 0b, 255b], [0b, 255b, 0b], [255b, 0b, 0b], [0b, 0b, 0b]]
  ...
  Endif Else Begin              ;no n_pot
      ...
      panel[4] -> getproperty, yaxis = yobj
      yobj -> setproperty, scaling = 1
      quick_set_panel_labels, panel[4], ['ESA Ni', 'ESA Ne', '[cm^-3]'],$
  ...
  yobj -> setproperty, scaling = 1
  quick_set_panel_labels, panel[6], ['Ti_par', 'Ti_perp', $
                                     'Te_par', 'Te_perp', $
                                     '[eV](ESA)'], $
                          colors_in = [[0b, 255b, 0b], [0b, 0b, 255b], $
                                       [0b, 0b, 0b], [255b, 0b, 0b], $
                                       [0b, 0b, 0b]]
  ...
  If(ntraces Gt 1) Then Begin
      ...
      for i = 0,n_elements(trace_obj)-1 do begin
          ...
          symbolobj->setProperty,show=1,name=symbolobj->getSymbolName(symbolid=4),id=4,color=linecolor
      endfor
  Endif
  
  quick_set_panel_labels, panel[7], ['Samp', 'Rate','','','']
...
If(is_string(tnames(thx+'_state_pos_gse'+osuffix))) Then Begin
    ...
    if ~keyword_set(no_draw) then begin
      ...
      For j = 0, 2 Do Begin
          to0 = obj_new('thm_ui_text')
          to0 -> setproperty, value = lbl[j]
          to0 -> setproperty, size = 8.0
          vo[j] = obj_new('thm_ui_variable')
