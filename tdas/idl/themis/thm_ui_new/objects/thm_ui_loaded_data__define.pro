...
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/thmsoc/tags/tdas_5_21/idl/themis/thm_ui_new/objects/thm_ui_loaded_data__define.pro $
;-----------------------------------------------------------------------------------
...
FUNCTION THM_UI_LOADED_DATA::Add, name, file=file, mission=mission,observatory=observatory,coordSys=coordSys,instrument=instrument,timerange=timerange,isSpect=isSpect,groupname=groupname,component_names=component_names,added_name=added_name,units=units,yaxisunits=yaxisunits,isYaxis=isYaxis,suffix=suffix,indepName=indepName,yaxisparent=yaxisparent
  ...
  if size(d.x,/type) ne 5 then begin
    ok = error_message('Cannot load "'+name+'". The x-axis must contain double precision floating point data.',/noname,/center, title='Data Load Error')
    return, 0
  endif
...
FUNCTION thm_ui_loaded_data::addData,in_name,d,limit=l,dlimit=dl,file=file, mission=mission,observatory=observatory,coordSys=coordSys,instrument=instrument,timerange=timerange,isSpect=isSpect,groupname=groupname,component_names=component_names,added_name=added_name,units=units,yaxisunits=yaxisunits,isYaxis=isYaxis,suffix=suffix,indepName=indepName,yaxisparent=yaxisparent
  ...
  if n_elements(timerange) eq 0 then begin
    tr = OBJ_NEW('thm_ui_time_range',startTime=d.x[0],endTime=d.x[n_elements(d.x)-1])
  endif else begin
    tr = OBJ_NEW('thm_ui_time_range',startTime=timerange[0],endTime=timerange[1])
  endelse 
 ...
 ;   prefix = strtrim(string(self.groupID),2) + ':'
    ...
    group = obj_new('thm_ui_data_group',name=groupname)
  
    timename = groupname+'_time'
    
    timeObj = obj_new('thm_ui_data',timename,self.dataId)
    
    dsettings = obj_new('thm_ui_data_settings',timename,0)
   ...
   ;if we need to create a yaxis quantity this code does it
    ...
    for i = 0,n_elements(component_names)-1 do begin
      ...
      dataObj = obj_new('thm_ui_data',component_names[i],self.dataId)    
      dsettings = obj_new('thm_ui_data_settings',isSpect?name:component_names[i],i)
      dsettings->fromLimits,l,dl
...
pro thm_ui_loaded_data::detectDlimits,name,dl,mission=mission,observatory=observatory,instrument=instrument,file=file,coordsys=coordsys,units=units
...
function THM_UI_LOADED_DATA::detectMission, name, dl

  compile_opt idl2, hidden
  
  ; check if dlimits exists
  if ~(size(dl, /type) eq 8) then begin
    dprint, 'Unable to determine mission/project for ' + name
    mission='unknown'
...
function THM_UI_LOADED_DATA::detectObservatory, name, dl, mission

  compile_opt idl2, hidden

  ; check if dlimits exists
  if ~(size(dl, /type) eq 8) then begin
    dprint, 'Unable to determine observatory for ' + name
    observatory = 'unknown'
    return, observatory
  endif
...
function THM_UI_LOADED_DATA::detectInstrument, name, dl,mission
  ...
  ; check if dlimits exists
  if ~(size(dl, /type) eq 8) then begin
    dprint, 'Unable to determine instrument type for ' + name
    return, instrument
  endif
        ...
        if in_set('data_type', strlowcase(tag_names(dl.cdf.gatt))) then begin
          ...
          case inst_string of
            ...
            'STA': begin
              ...
              efi_names = thm_ui_new_valid_datatype('efi')
              ...
              fft_names = thm_ui_new_valid_datatype('fft')
              ...
              scm_names = thm_ui_new_valid_datatype('scm')
  ...
  if ~success then begin
    if mission eq 'THEMIS' then begin
      if stregex(name,'^thg_ask_.*',/boolean,/fold_case) then begin
        ...
        instrument = 'fbk'
        success = 1
      endif else if stregex(name,'^th[abcde]_ff[pw]_16.*',/boolean,/fold_case) || $
                    stregex(name,'^th[abcde]_ff[pw]_32.*',/boolean,/fold_case) || $
                    stregex(name,'^th[abcde]_ff[pw]_64.*',/boolean,/fold_case) then begin
        instrument = 'fft'
...
function THM_UI_LOADED_DATA::detectFilename, name, dl

  compile_opt idl2, hidden

  ; check if dlimits exists
  if ~(size(dl, /type) eq 8) then begin
    dprint, 'Unable to determine source file for ' + name
    file = 'unknown'
    return, file
  endif
...
function THM_UI_LOADED_DATA::detectCoordsys, name, dl

  compile_opt idl2, hidden

  ; check if dlimits exists
  if ~(size(dl, /type) eq 8) then begin
    dprint, 'Unable to determine coordinate system for ' + name
    coordsys = 'unknown'
    return, coordsys
  endif
...
function THM_UI_LOADED_DATA::detectunits, name, dl
 ...
 ;   dprint, 'Unable to determine units for ' + name
  ...
  if in_set('data_att', strlowcase(tag_names(dl))) && is_struct(dl.data_att) then begin
    ...
    if in_set('units', strlowcase(tag_names(dl.data_att))) then begin
      
      units = dl.data_att.units
      success = 1
    endif
  endif
...
function THM_UI_LOADED_DATA::addTvarObject,obj,component_names=component_names,added_name=added_name
...
FUNCTION THM_UI_LOADED_DATA::Remove, name,nodelete=nodelete
...
FUNCTION THM_UI_LOADED_DATA::GetAll, Parent=parent,Child=Child,Times=times
...
FUNCTION THM_UI_LOADED_DATA::GetActive, Parent=parent,Child=child
...
function THM_UI_LOADED_DATA::getChildren,name
...
PRO THM_UI_LOADED_DATA::SetActive, name
...
FUNCTION THM_UI_LOADED_DATA::IsActive,name
...
FUNCTION THM_UI_LOADED_DATA::IsChild,name
...
FUNCTION THM_UI_LOADED_DATA::IsParent,name
...
PRO THM_UI_LOADED_DATA::ClearActive, thisName
...
PRO THM_UI_LOADED_DATA::ClearAllActive
...
Pro THM_UI_LOADED_DATA::GetVarData,name=name,time=t,data=d,yaxis=y,limits=l,dlimits=dl,mission=mission,observatory=observatory,coordsys=coordsys,instrument=instrument,trange=trange,units=units,isTime=isTime,tmName=timeName,duplicate=duplicate
...
function THM_UI_LOADED_DATA::GetTvarData,name
...
pro thm_ui_loaded_data::getdatainfo,name,mission=mission,observatory=observatory,instrument=instrument,units=units,coordinate_system=coordinate_system,filename=filename,fail=fail
  ...
  reference->getProperty,mission=mission,observatory=observatory,instrument=instrument,units=units,coordSys=coordinate_system,filename=filename
...
pro thm_ui_loaded_data::setDataInfo,name,newname=newname,mission=mission,observatory=observatory,instrument=instrument,units=units,coordinate_system=coordinate_system,windowstorage=windowstorage,fail=fail
...
PRO THM_UI_LOADED_DATA::Cleanup
...
function THM_UI_LOADED_DATA::GetTvarObject,name
  ...
  return,obj_new('thm_ui_data',varname,-1, $
    groupname=groupname, $
    filename=filename, $
    isTime=isTime,     $
    isYaxis=isYaxis,   $
...
FUNCTION THM_UI_LOADED_DATA::GetObjects,name=name
...
function thm_ui_loaded_data::getGroup,name
...
function thm_ui_loaded_data::getGroupNames
...
;pro thm_ui_loaded_data::SplitVecPtr,name,outnames=outnames,outptrs=outptrs,suffixes=suffixes
pro thm_ui_loaded_data::SplitVecPtr,name,d,outnames=outnames,outptrs=outptrs,suffixes=suffixes
...
function thm_ui_loaded_data::getTreeData
...
FUNCTION THM_UI_LOADED_DATA::Init
...
PRO THM_UI_LOADED_DATA__DEFINE

   struct = { THM_UI_LOADED_DATA,    $
