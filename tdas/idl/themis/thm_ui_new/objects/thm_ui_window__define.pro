...
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/thmsoc/tags/tdas_5_21/idl/themis/thm_ui_new/objects/thm_ui_window__define.pro $
;-----------------------------------------------------------------------------------
...
;This method originally created for use with thm_ui_lock_axes
function thm_ui_window::getColumn,in_col
...
;2: middle
;3: top
function thm_ui_window::getPanelPos,panel
...
pro thm_ui_window::repack,nodecrease=nodecrease
  ...
  if ~obj_valid(settings) || ~obj_isa(settings,'thm_ui_panel_settings') then begin
...
;pro thm_ui_window::setAutoTicks,val,xaxis=xaxis,yaxis=yaxis,zaxis=zaxis
;
;  compile_opt idl2
;  
;  if obj_valid(self.panels) && $
...
;;of the window
;pro thm_ui_window::syncTicks,copy
;
;  compile_opt idl2
;  
...
pro thm_ui_window::updatedatareference,oldnames,newnames
...
function thm_ui_window::getMargins
...
FUNCTION THM_UI_WINDOW::Copy
   out = Obj_New("THM_UI_WINDOW", 1)
...
FUNCTION THM_UI_WINDOW::GetPanelId
RETURN, self.panelId
END ;--------------------------------------------------------------------------------

PRO THM_UI_WINDOW::IncrementPanelId
self.panelId = self.panelId + 1
END ;--------------------------------------------------------------------------------

pro thm_ui_window::save
...
FUNCTION THM_UI_WINDOW::Init,    $ ; The INIT method of the line style object
   ...
   IF NOT Obj_Valid(settings) THEN settings = Obj_New('THM_UI_PAGE_SETTINGS')
...
PRO THM_UI_WINDOW__DEFINE

   struct = { THM_UI_WINDOW,          $
