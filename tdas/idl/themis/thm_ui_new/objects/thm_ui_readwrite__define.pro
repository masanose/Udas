...
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/thmsoc/tags/tdas_5_21/idl/themis/thm_ui_new/objects/thm_ui_readwrite__define.pro $
;-----------------------------------------------------------------------------------


FUNCTION THM_UI_READWRITE::Init
                   
RETURN, 1
END

;helper function to add line feeds/carriage returns to document
pro thm_ui_readwrite::appendXMLNewline,parent
...
function thm_ui_readwrite::getNodeArray,nodelist
...
function thm_ui_readwrite::GetDOMElement,parent_node
...
pro thm_ui_readwrite::BuildFromDOMElement,dom_element
type_strings=['undefined','byte','int','long','float','double','complex', $
             'string','struct','dcomplex','pointer','object','uint','ulong',$
             'long64','ulong64']
format_strings=['(I)','(I)','(I)','(I)','(E14.6)','(E21.13)','(D)',$
...
;print,'In BuildFromDOMElement: '+element_name
...
;print,att_count,format='("Found ",I," attributes:")'
for i=0, att_count-1 do begin
  ...
  ;print,name,type,nelem,format='("Name: ",A," Type: ",A," nelem: ",A)'
  ...
  for j=0,proto_nelem-1 do begin
    ...
    if (is_primitive[proto_type]) then begin
       ...
       ;print,'Value '+valindexstr+' '+valstring+' '+fmt
       reads,valstring,proto_variable,format=fmt
       ;print,'converted value: '+string(proto_variable,format=fmt)+' index: '+string(valindex,format='(I)')
       ; Do the assignment
       self.(tagindex)[valindex] = proto_variable
        ...
        if (classname EQ 'NULLOBJ') then begin
            ...
            ;print,'Assigning null object reference'
           ...
           ;print,'Making IDL_CONTAINER of '+string(container_count)+' objects, tclass '+contained_type
             ...
             while obj_valid(container_sib) do begin
                 ...
                 if (sibname EQ 'value') then begin
                    ...
                    if (tgt_type EQ contained_type) then begin
                       ...
                       ;print,'Loading IDL_CONTAINER, ignored node name '+tgt_type
                    endelse
                 endif else begin
                    ;print,'Loading IDL_CONTAINER, ignored node name '+sibname
                 endelse
...
pro thm_ui_readwrite::write,lun
...
;
        ...
        ; Get object type from RHS, ensure that it matches field type
           ...
           ;
           ; General case: THM_UI class with a read method and no-argument
           ; constructor
           ;
...
; Scalar primitives
;
        ;print,'Field assignment: '+inputline
...
PRO THM_UI_READWRITE__DEFINE

   struct = { THM_UI_READWRITE, __dummy:0 }
