...
;$LastChangedDate: 2010-02-17 13:39:47 -0800 (Wed, 17 Feb 2010) $
;$LastChangedRevision: 7291 $
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/thmsoc/tags/tdas_5_21/idl/themis/thm_ui_new/objects/thm_ui_history__define.pro $
;+ 
;NAME: 
...
PRO thm_ui_history_event, event
...
pro thm_ui_history::Save , filename=filename,append=append

xt = time_string(systime(/sec))
ttt = strmid(xt, 0, 4)+strmid(xt, 5, 2)+strmid(xt, 8, 2)+'_'+strmid(xt,11,2)+strmid(xt,14,2)+strmid(xt,17,2)
historyFile = 'thm_gui_history_'+ttt+'.txt'
if size(filename,/type) ne 7 then begin
  fileName = dialog_pickfile(title = 'THEMIS GUI History Filename', filter = '*.txt', file = historyFile, /Write)
endif
...
PRO THM_UI_HISTORY::Draw
  ...
  IF self.id EQ 0 or self.state EQ 0 THEN BEGIN
      ...
      tlb = Widget_Base(/Col, Title='THEMIS: History Window', Group_Leader=self.mainID, /tlb_kill_request_events)
      msgs=*self.messages
      ...
      XManager, 'thm_ui_history', tlb, /No_Block
...
PRO THM_UI_HISTORY::Update, message, DontShow=dontShow,nosave=nosave
...
if ~nosave then begin
  ...
  self->save,filename=self.running_history_dir+'/thm_gui_running_history.txt',append=append
endif
END ;--------------------------------------------------------------------------------



FUNCTION THM_UI_HISTORY::GetState
RETURN, self.state
END ;--------------------------------------------------------------------------------



FUNCTION THM_UI_HISTORY::ReadJournalFile
  if logical_true(!journal) then begin
    running_history_dir = self.running_history_dir
    journalFile = running_history_dir+'/thm_ui_idlsave.pro'
...
PRO THM_UI_HISTORY::SetProperty,  $ ; The property set method for the object
...
PRO THM_UI_HISTORY::GetProperty, $
...
pro thm_ui_history::saveHistoryFile
  ...
  while ~self.lastSaveSuccess do begin
    result = dialog_message("The running history file has unsaved entries, and may be open in another application.  "+ $
      "Please close the file 'THM_GUI_RUNNING_HISTORY.TXT in any other applications, and click 'OK'.  "+ $
...
FUNCTION THM_UI_HISTORY::Init,       $ ; The INIT method of the bar object.
...
PRO THM_UI_HISTORY__DEFINE

   struct = { THM_UI_HISTORY,          $
