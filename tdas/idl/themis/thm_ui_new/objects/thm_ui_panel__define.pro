...
;$LastChangedDate: 2010-03-25 10:04:20 -0700 (Thu, 25 Mar 2010) $
;$LastChangedRevision: 7439 $
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/thmsoc/tags/tdas_5_21/idl/themis/thm_ui_new/objects/thm_ui_panel__define.pro $
;-----------------------------------------------------------------------------------

FUNCTION THM_UI_PANEL::Copy
   out = Obj_New("THM_UI_PANEL", self.id,/nosave)
   Struct_Assign, self, out
   ; copy panel settings
   newSettings=Obj_New("THM_UI_PANEL_SETTINGS")
   IF Obj_Valid(self.Settings) THEN newSettings=self.settings->Copy() ELSE $
      newSettings=Obj_New()
   out->SetProperty, Settings=newSettings
   ; copy x axis
   newAxis=Obj_New("THM_UI_AXIS_SETTINGS")
   ...
   ; copy z axis
   newAxis=Obj_New("THM_UI_ZAXIS_SETTINGS")
   ...
   newVariable=Obj_New("THM_UI_VARIABLE")
   ...
   newMarker=Obj_New("THM_UI_MARKER")
...
FUNCTION THM_UI_PANEL::GetPanelCoordinates
...
pro THM_UI_PANEL::setPanelCoordinates,coord
...
function thm_ui_panel::constructPanelName
...
function thm_ui_panel::constructTraceNames
  ...
  if obj_valid(self.traceSettings) AND $
    ...
    names = strarr(n_elements(objs))
      ...
      for i = 0,n_elements(objs)-1 do begin
        ...
        if obj_isa(objs[i],'thm_ui_line_settings') then begin
          objs[i]->getProperty,dataX=dataX,dataY=dataY
          
          names[i] = '  - ' + dataX + ' -vs- ' + dataY
        
        endif else if obj_isa(objs[i],'thm_ui_spectra_settings') then begin
...
pro thm_ui_panel::updatedatareference,oldnames,newnames
...
PRO THM_UI_PANEL::SyncLinesToLabels
   ...
   FOR i=0,nLines-1 DO BEGIN
      ...
      IF Obj_Isa(settings[i], 'THM_UI_LINE_SETTINGS') THEN BEGIN   ;is a line
...
PRO THM_UI_PANEL::SyncLabelsToLines, oldnames=oldnames, newnames=newnames
   ...
   if keyword_set(oldnames) && keyword_set(newnames) then begin
     ...
     for i = 0,n_elements(traceSettings)-1 do begin
       ...
       if obj_valid(tracesettings[i]) then begin
         ...
         if obj_isa(tracesettings[i],'thm_ui_spectra_settings') then begin
...
FUNCTION THM_UI_PANEL::GetLayoutStructure
...
pro THM_UI_PANEL::SetLayoutStructure, Row=row, Col=col
self.settings->SetProperty, Row=row, Col=col
END ;--------------------------------------------------------------------------------

PRO THM_UI_PANEL::Save
...
FUNCTION THM_UI_PANEL::Init,    $ ; The INIT method of the line style object
   ...
   IF NOT Obj_Valid(settings) THEN settings = Obj_New('THM_UI_PANEL_SETTINGS')
