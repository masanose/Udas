...
FUNCTION THM_UI_PANEL_SETTINGS::Copy
   out = Obj_New("THM_UI_PANEL_SETTINGS",/nosave)
...
;handles special cases for setProperty, the rest is handled by parent class: thm_ui_getset
pro thm_ui_panel_settings::setProperty,$
                             bunit=bunit,$
                             lunit=lunit,$
                             wunit=wunit,$
                             hunit=hunit,$
  ...
  self->thm_ui_getset::setProperty,bunit=bunit,lunit=lunit,wunit=wunit,hunit=hunit,_extra=ex
  
end

FUNCTION THM_UI_PANEL_SETTINGS::GetUnitNames
RETURN, ['pt', 'in', 'cm', 'mm']
END ;--------------------------------------------------------------------------------



FUNCTION THM_UI_PANEL_SETTINGS::GetUnitName, index
names = self->GetUnitNames()
RETURN, names[index]
END ;--------------------------------------------------------------------------------

function THM_UI_PANEL_SETTINGS::convertunit,value,oldunit,newunit
...
PRO THM_UI_PANEL_SETTINGS::Save
...
PRO THM_UI_PANEL_SETTINGS::Reset
...
pro thm_ui_panel_settings::getProperty,backgroundrgb=backgroundrgb,_ref_extra=ex
...
FUNCTION THM_UI_PANEL_SETTINGS::Init,     $
   ...
   IF N_Elements(titleObj) EQ 0 THEN titleObj = obj_new('thm_ui_text', size=10)
...
PRO THM_UI_PANEL_SETTINGS__DEFINE

   struct = { THM_UI_PANEL_SETTINGS, $
