...
;$LastChangedDate: 2010-02-08 14:59:32 -0800 (Mon, 08 Feb 2010) $
;$LastChangedRevision: 7229 $
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/thmsoc/tags/tdas_5_21/idl/themis/thm_ui_new/objects/thm_ui_widget_tree__define.pro $
;
;--------------------------------------------------------------------------------
...
pro thm_ui_widget_tree::update,selected=selected,from_copy=copy
  compile_opt idl2
...
pro thm_ui_widget_tree::getProperty,$
                                uvalue=uvalue,$
                                uname=uname,$
                                xsize=xsize,$
                                ysize=ysize,$
...
;  showdatetime: If this boolean is 1 the date time of a variable will
;             be printed with its name
pro thm_ui_widget_tree::setProperty,$
                                uvalue=uvalue,$
                                uname=uname,$
                                xsize=xsize,$
                                ysize=ysize,$
...
;copy the tree
function thm_ui_widget_tree::getCopy
...
function thm_ui_widget_tree::getValue
...
;getValue method
function thm_ui_widget_tree::getValueRecursive,widget,selected
...
function thm_ui_widget_tree::getValueGroup,widget,selected
...
function thm_ui_widget_tree::getGroupValueRecursive,widget,selected
...
function thm_ui_widget_tree::formatValueGroup,names
...
;returned
pro thm_ui_widget_tree::collapseNode,node
...
pro thm_ui_widget_tree::copyexpansion,oldnode,newnode
...
pro thm_ui_widget_tree::populateTree,oldtree

  compile_opt idl2

  self.root = widget_tree(self.stash,xsize=self.xsize,ysize=self.ysize,UVALUE={value:'INTERIOR',children:'',name:''},multiple=self.multi,bitmap=self.nodemap,/mask)
  ...
  if is_struct(ldTreeData) then begin
    ...
    for i = 0,dim[0]-1 do begin
      ...
      self->addBranch,ldTreeData.names[i,*],ldTreeData.objs[i],oldtree
...
pro thm_ui_widget_tree::addBranch,names,obj,oldtree,force=force,addchild=addchild,addyaxis=addyaxis
...
pro thm_ui_widget_tree::addChildren,parent,object,ygroup=ygroup,yobject=yobject
...
pro thm_ui_widget_tree::addGroup,parent,object,group=group
...
function thm_ui_widget_tree::getChild,widget,name
...
pro thm_ui_widget_tree::expandInterior,node,name,next_node=next_node
...
pro thm_ui_widget_tree::expandNode,node
...
pro thm_ui_widget_tree::addDummy,node
...
function thm_ui_widget_tree::hasDummy,node
...
pro thm_ui_widget_tree::removeDummy,node
...
pro thm_ui_widget_tree::setSelected,selected
...
pro thm_ui_widget_tree::setSelectedRecursive,node,selected
...
pro thm_ui_widget_tree::clearSelected
...
function thm_ui_widget_tree::getDataChildren,widget,names=names
...
;widgets
function thm_ui_widget_tree::handleEvent,event
  ...
  if Tag_Names(event, /Structure_Name) EQ 'WIDGET_TREE_EXPAND' then begin
...
;    return,{THM_UI_TREE_EXPAND,ID:self.id,TOP:event.top,HANDLER:0L}
    return,0
    ...
    return,{THM_UI_TREE_EVENT,ID:self.id,TOP:event.top,HANDLER:0L}
...
function thm_ui_widget_tree::cleanup
...
function thm_ui_widget_tree::init,$
                           parentid,$
                           uvalue,$
                           loadedData,$
                           uname=uname,$
  ...
  self.id = widget_base(self.parent, $
                uvalue=uvalue, $
                uname=uname, $
                event_func='thm_ui_widget_tree_event',$
                func_get_value='thm_ui_widget_tree_get_value')
...
function thm_ui_widget_tree_event,event
...
;NOTE: this routine should only be called internally,
;it should not be called by the end-user.
function thm_ui_widget_tree_get_value,id
...
pro thm_ui_widget_tree__define

  compile_opt idl2

  struct = { THM_UI_WIDGET_TREE,     $
