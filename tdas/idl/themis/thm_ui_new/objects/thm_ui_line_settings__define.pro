...
FUNCTION THM_UI_LINE_SETTINGS::Copy
   out = Obj_New("THM_UI_LINE_SETTINGS",/nosave)
   ...
   newLineStyle=Obj_New("THM_UI_LINE_STYLE")
...
function thm_ui_line_settings::getPtSpacing
...
pro thm_ui_line_settings::updatedatareference,oldnames,newnames,changed=changed
...
FUNCTION THM_UI_LINE_SETTINGS::GetUnits
RETURN, ['sec', 'minutes', 'hours', 'days', '', '<none>'] 
END ;--------------------------------------------------------------------------------



FUNCTION THM_UI_LINE_SETTINGS::GetUnit, index
...
FUNCTION THM_UI_LINE_SETTINGS::GetPlotOptions
RETURN, ['All Points', 'First and Last Points', 'First Point', 'Last Point', 'Major Ticks', 'Every']
END ;--------------------------------------------------------------------------------



FUNCTION THM_UI_LINE_SETTINGS::GetPlotOption, index
  IF N_Elements(index) EQ 0 THEN RETURN, 0
  IF NOT Is_Numeric(index) THEN RETURN, 0
  IF index LT 0 OR index GT 5 THEN RETURN, 0
...
PRO THM_UI_LINE_SETTINGS::Reset
...
FUNCTION THM_UI_LINE_SETTINGS::Init,           $
   ...
   IF NOT Obj_Valid(linestyle)THEN linestyle = Obj_New("THM_UI_LINE_STYLE")
   IF NOT Obj_Valid(symbol)THEN symbol = Obj_New("THM_UI_SYMBOL")
   IF NOT Obj_Valid(barline)THEN barline = Obj_New("THM_UI_LINE_STYLE", show=0)
   IF NOT Obj_Valid(marksymbol)THEN marksymbol = Obj_New("THM_UI_LINE_STYLE")
...
PRO THM_UI_LINE_SETTINGS__DEFINE

   struct = { THM_UI_LINE_SETTINGS, $
              ...
              inherits thm_ui_getset $  ;generalized getProperty,setProperty,getAll,setAll methods
