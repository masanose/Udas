...
pro thm_ui_interp_match_event, event
  ...
  if on_err ne 0 then begin
    ...
    ok=error_message('An unkown error occured and the window must be restarted, see console for details.', $
  ...
  if tag_names(event, /structure_name) eq 'WIDGET_KILL_REQUEST' then begin
        state.historywin->update,'THM_UI_INTERPOL_OPTIONS: Match window killed',/dontshow
        widget_control, event.top, /destroy
        return
  endif
  ...
  if size(uval, /type) ne 0 then begin
    case uval of
      ...
      'CANCEL':begin
        ...
        state.historywin->update,'THM_UI_INTERPOL_OPTIONS: Match window canceled'
        widget_control, event.top, /destroy
        return
      end
...
function thm_ui_interp_match, state
...
;Widget Tree
  tree = obj_new('thm_ui_widget_tree', mtlb, 'TREE', *state.datap, xsize=300, $
                 ysize=400, mode=0)
...
;Initialize
  ...
  xmanager, 'thm_ui_interp_match', mtlb, /no_block
...
pro thm_ui_interp_updatestatus, state
...
pro thm_ui_interpol_options_event, event
...
;kill requests
  if tag_names(event,/struc) eq 'WIDGET_KILL_REQUEST' then begin
    state.historywin->update,'THM_UI_INTERPOL_OPTIONS: Widget killed', /dontshow
    widget_control, event.top, set_uval=state, /no_copy
    widget_control, event.top, /destroy
    return
  endif
  ...
  if size(uval,/type) ne 0 then begin
    Case uval Of
      'OK':begin
        ...
        if finite(num) && (num gt 0) then (*state.pvals).num = num $
          else if (*state.pvals).ntype eq 0 then begin
            ok = dialog_message('Invalid number of points, please re-try',/information, $
        ...
        if finite(cad) && (cad gt 0) then (*state.pvals).cad = cad $
            ...
            ok = dialog_message('Invalid cadence, please re-try',/information, $
                                /center,title='Interpolate Error')
            break
          endif
        ...
        ;Get type requested (0=linear, 1=quad, 2=lst sqr qd, 3=spline)
        ...
        if (*state.pvals).limit then begin
          widget_control, state.trange, get_value=tr
          (*state.pvals).trange = tr
        endif
      ...
      'NUM': thm_ui_interp_updatestatus, state

      'CAD': thm_ui_interp_updatestatus, state

      'NUMB': begin
        widget_control, state.cadtext, sens=0
        widget_control, state.numtext, sens=1
        thm_ui_interp_updatestatus, state
      end
      ...
      'MATCHTO':begin
        ...
        name = thm_ui_interp_match(state)
...
;    type: type on interpolation (0=linear, 1=quad, 2=lst sqr qd, 3=spline)
...
function thm_ui_interpol_options, gui_ID, historywin, statusbar, datap, ptree = ptree
...
;Number widgets
  ...
  numtext = thm_ui_spinner(spinnerbase, text_box_size=10, $
                           uval='NUM', value = num, incr=1)
  cadtext = thm_ui_spinner(spinnerbase, text_box_size=10, sens=0, $
                           uval='CAD', value = cad, incr=0.1)

;Type widgets
  linear = widget_button(typebase, value = 'Linear')
  quad = widget_button(typebase, value = 'Quadradic')
...
;Time range widgets
  ...
  time = thm_ui_time_widget(timebase, statusbar, historywin, oneday=0b)
...
;
  ...
  endif else tr = obj_new('THM_UI_TIME_RANGE')

;struct to be returned later
values = {num:num, cad:cad, type:[0,0,0,0], matchto:'', match:0b, ntype:0b, $
  ...
  state = {tlb:tlb, gui_id:gui_id, historywin:historywin, statusbar:statusbar, $
           type:[linear,quad,lsquad,spline], extra:[noextra,extra,nanextra], ntype:number, $
  ...
  xmanager, 'thm_ui_interpol_options', tlb, /no_block
