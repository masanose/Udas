...
pro thm_ui_high_pass_options_event, event
...
;error catch block
  ...
  if on_err ne 0 then begin
    ...
    ok=error_message('An unkown error occured and the window must be restarted, '+ $
...
;kill requests
  if tag_names(event,/struc) eq 'WIDGET_KILL_REQUEST' then begin
    state.historywin->update,'THM_UI_HIGH_PASS_OPTIONS: Widget killed', /dontshow
    ...
    Case uval Of
      'OK': begin
        ...
        if finite(ave) && (ave gt 0) then begin
          ...
          ok = dialog_message('Invalid averaging time, please re-enter', $
        ...
        if widget_info(state.icad, /sensitive) then begin
          ...
          if finite(icad) && (icad gt 0) then begin
            ...
            ok = dialog_message('Invalid interpolation cadence, please re-enter', $
                                /center,title='High Pass Filter Error')
            break
          endelse
        endif
      ...
      'CANCEL': begin
        state.historywin->update,'THM_UI_HIGH_PASS_OPTIONS: Widget canceled', /dontshow
        state.statusbar->update,'High Pass Filter canceled'
        widget_control, event.top, /destroy
        return
      end
      ...
      'AVE': begin ;update status bar
        ...
        else state.statusbar->update,'Invalid averaging value, please re-enter.' 
      end

      'ICAD': begin ;update status bar
        if event.valid && event.value gt 0 then state.statusbar->update, $
          m+'Interpolation cadence set to '+strtrim(event.value,2)+' seconds' $
        else state.statusbar->update,'Invalid interpolation cadence, please re-enter.'
      end
...
function thm_ui_high_pass_options, gui_id, statusbar, historywin
...
;Options Widgets
  averageRes = thm_ui_spinner(avebase, label='Averaging Time (sec): ', incr=1, $
...
;Initializations
  ...
  xmanager, 'thm_ui_high_pass_options', tlb, /no_block
