...
pro thm_ui_time_derivative_options_event, event
...
;kill requests
  if tag_names(event,/struc) eq 'WIDGET_KILL_REQUEST' then begin
    state.historywin->update,'THM_UI_TIME_DERIVATIVE_OPTIONS: Widget killed', /dontshow
...
;get user value
  ...
  if size(uval,/type) ne 0 then begin
    Case uval Of
      ...
      'CANCEL': begin
        state.historywin->update,'THM_UI_TIME_DERIVATIVE_OPTIONS: Cancelled', /dontshow
        state.statusbar->update,'Time derivative canceled'
        widget_control, event.top, /destroy
        return
      end
...
function thm_ui_time_derivative_options, gui_id, statusbar, historywin
...
;Options widgets
  ...
  smoothwidth = thm_ui_spinner(smoothBase,value=swidth,text_box_size=10,uval='SMOOTH', $
                               tooltip='The width of the smoothing window, in elements', $
                               label='   Smoothing Width(# Pts): ', incr=1, sens=0)
...
;Initializations
  ...
  xmanager, 'thm_ui_time_derivative_options', tlb, /no_block
