...
pro thm_ui_clean_spikes_options_event, event
...
;kill requests
  if tag_names(event,/struc) eq 'WIDGET_KILL_REQUEST' then begin
    state.historywin->update,'THM_UI_CLEAN_SPIKES_OPTIONS: Widget killed', /dontshow
    ...
    Case uval Of
      ...
      ;Get threshold
        ...
        if finite(thresh) && (thresh ge 1) then begin
          ...
          ok = dialog_message('Invalid threshold, please re-enter.', $
      ...
      ;Get smoothing width
        ...
        if finite(swidth) && (swidth gt 0) then begin
          ...
          ok = dialog_message('Invalid smoothing width, please re-enter.', $
                              /center,title='Clean Spikes Error')
          break
        endelse
      ...
      'CANCEL': begin
        state.historywin->update,'THM_UI_CLEAN_SPIKES_OPTIONS: Cancelled', /dontshow
        state.statusbar->update,'Clean Spikes canceled'
        widget_control, event.top, /destroy
        return
      end
      'THRESH': begin
        ...
        else state.statusbar->update,'Invalid threshold, please re-enter.'
      end
      'SMOOTH': begin
        if event.valid && event.value gt 0 then state.statusbar->update, $
          m+'Smoothing window set to '+strtrim(event.value,2)+' points.' $
        else state.statusbar->update,'Invalid smoothing window, please re-enter.'
      end
      else: 
    Endcase
  endif
...
function thm_ui_clean_spikes_options, gui_id, statusbar, historywin
...
;Options widgets
  smoothwidth = thm_ui_spinner(smoothBase,value=swidth,label='Smoothing Width(# Pts):  ',text_box_size=10, $
                               uval='SMOOTH',incr=1,tooltip='The width, in elements, of the '+ $
                               'smoothing window', getxlabelsize=xlsize)

  threshnumber = thm_ui_spinner(threshBase,value=thresh,label='Threshold:  ',text_box_size=10, $
                                uval='THRESH',incr=1,tooltip='Set lower for more liberal determination of spikes, [1,infinity)', xlabelsize=xlsize)
...
;Initializations
  ...
  xmanager, 'thm_ui_clean_spikes_options', tlb, /no_block
