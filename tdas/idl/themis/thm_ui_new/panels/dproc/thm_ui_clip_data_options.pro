...
pro thm_ui_clip_data_options_event, event
...
;kill requests
  if tag_names(event,/struc) eq 'WIDGET_KILL_REQUEST' then begin
    state.historywin->update,'THM_UI_clip_DATA_OPTIONS: Widget killed', /dontshow
...
function thm_ui_clip_data_options, gui_id, statusbar, historywin
...
;Options Widgets
  maxclip = thm_ui_spinner(maxminbase, value=maxc, text_box_size=10, incr=1, $
                           uval='MAX', label='Maximum for Clip:  ', getxlabelsize=xlsize, $
                           tooltip='Values greater than the maximum will be clipped')
  minclip = thm_ui_spinner(maxminbase, value=minc, text_box_size=10, incr=1, $
                           uval='MIN', label='Minimum for Clip:  ', xlabelsize=xlsize, $
  ...
  flag = thm_ui_spinner(flagBase, value=0, text_box_size=10, incr=1, $
...
;Initializations
  ...
  xmanager, 'thm_ui_clip_data_options', tlb, /no_block
