...
pro thm_ui_deflag_options_event, event
...
;kill requests
  if tag_names(event,/struc) eq 'WIDGET_KILL_REQUEST' then begin
    state.historywin->update,'THM_UI_DEFLAG_OPTIONS: Widget killed', /dontshow
...
;user value for case statement
  ...
  if size(uval,/type) ne 0 then begin
    Case uval Of
      ...
      'CANCEL': begin
        state.historywin->update,'THM_UI_DEFLAG_OPTIONS: Widget canceled', /dontshow
        state.statusbar->update,'Deflag canceled'
        widget_control, event.top, /destroy
        return
      end
...
;+
;NAME:
;  thm_ui_deflag_options
;
;PURPOSE:
;  Front end interface allowing the user to select options for deflagging data.
;
;CALLING SEQUENCE:
;  return_values = thm_ui_deflag_options(gui_id, statusbar, historywindow)
;
;INPUT:
...
function thm_ui_deflag_options, gui_id, statusbar, historywin
...
;Initializations
  ...
  xmanager, 'thm_ui_deflag_options', tlb, /no_block
