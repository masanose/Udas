...
pro thm_ui_block_ave_options_event, event
...
;kill requests
  if tag_names(event,/struc) eq 'WIDGET_KILL_REQUEST' then begin
    state.historywin->update,'THM_UI_BLOCK_AVE_OPTIONS: Widget killed', /dontshow
...
;user value for case statement
  ...
  if size(uval,/type) ne 0 then begin
    Case uval Of
      ...
      'CANCEL': begin
        state.historywin->update,'THM_UI_BLOCK_AVE_OPTIONS: Widget canceled', /dontshow
        state.statusbar->update,'Block Average canceled'
        widget_control, event.top, /destroy
        return
      end
...
function thm_ui_block_ave_options, gui_id, statusbar, historywin, datap
...
;Options Widgets
  resolution = thm_ui_spinner(resbase, label = 'Time Resolution (sec):  ', $
...
;Initializations
  ...
  xmanager, 'thm_ui_block_ave_options', tlb, /no_block
