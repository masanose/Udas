...
;$LastChangedDate: 2009-05-22 16:05:25 -0700 (Fri, 22 May 2009) $
;$LastChangedRevision: 5953 $
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/thmsoc/tags/tdas_5_21/idl/themis/thm_ui_new/panels/thm_ui_image_export_options.pro $
;
;--------------------------------------------------------------------------------

pro thm_ui_image_export_options_event,event
  ...
  IF (err_xxx NE 0) THEN BEGIN
    ...
    ok=error_message('An unknown error occured and the window must be restarted. See console for details.',$
       /noname, /center, title='Error in Image Export Options')
  ...
  state.historywin->update,'THM_UI_IMAGE_EXPORT_OPTIONS: User value: '+uval  ,/dontshow
...
function thm_ui_image_export_options,gui_id,format,drawObject,options, historywin
  ...
  if format eq '.eps' || format eq '.emf' then begin
    ...
    ok_button = widget_button(row4,value='OK',uvalue='OK')
    canc_button = widget_button(row4,value='Cancel',uvalue='CANC')  
    ...
    if format eq '.gif' || format eq '.pic' then begin
    ...
    endif else if format eq '.ppm' || format eq '.srf' then begin
      ...
      row3 = widget_base(tlb,/row,/nonexclusive)
      ascii = widget_button(row3,value='Ascii',UVALUE='ASCII')
      
      if asc then begin
        widget_control,ascii,/set_button
      endif
      
      str_element,out_struct,'ascii',asc,/add
  ...
  XManager, 'thm_ui_image_export_options', tlb, /No_Block
