...
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/thmsoc/tags/tdas_5_21/idl/themis/thm_ui_new/panels/thm_ui_load_data_file/thm_ui_load_wind_data.pro $
;
;--------------------------------------------------------------------------------
pro thm_ui_load_wind_data_event,event
  ...
  if is_string(uval) then begin
    case uval of
      ...
      'ADD': begin
        ...
        state.callSequence->addloadwind,$
                               instrumentText,$
                               typeText,$
                               paramText,$
                               [startTimeString,endTimeString]
...
pro thm_ui_load_wind_data,tabid,loadedData,historyWin,statusBar,treeCopyPtr,timeRangeObj,callSequence,loadTree=loadTree,timeWidget=timeWidget
  ...
  topBase = Widget_Base(tabid, /Row, /Align_Top, /Align_Left, YPad=1,event_pro='thm_ui_load_wind_data_event') 
 ...
 timeWidget = thm_ui_time_widget(selectionBase,$
  ...
  typeArray[3] = ptr_new(['k0','pm','elpd','sfpd','sfsp'])
  ...
  (*paramArray[3])[3] = ptr_new(['*',['TIME','ENERGY','PANGLE','INTEG_T','MAGF']])
  (*paramArray[3])[4] = ptr_new(['*',['TIME','FLUX','ENERGY']])
