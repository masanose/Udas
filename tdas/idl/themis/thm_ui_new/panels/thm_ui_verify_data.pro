...
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/thmsoc/tags/tdas_5_21/idl/themis/thm_ui_new/panels/thm_ui_verify_data.pro $
;
;--------------------------------------------------------------------------------

;updates a page with the new contents from the metadata element at index
pro thm_ui_update_verify,state
...
pro thm_ui_update_meta,state,element,value
...
pro thm_ui_verify_data_event,event
  ...
  state.historywin->update,'THM_UI_VERIFY_DATA: User value: '+uval  ,/dontshow
  ...
  CASE uval OF
    ...
    'NAME': BEGIN
      ...
      if current_select[0] ne -1 && n_elements(current_select) eq 1 then begin
        thm_ui_update_meta,state,'name',value
    ...
    'MISSION': BEGIN
      ...
      thm_ui_update_meta,state,'mission',value
    END
    'OBSERVATORY': BEGIN
      Widget_Control, state.textWidgets[2], Get_Value=value
      thm_ui_update_meta,state,'observatory',value
    END
    'INSTRUMENT': BEGIN
      Widget_Control, state.textWidgets[3], Get_Value=value
      thm_ui_update_meta,state,'instrument',value
      
    END
    'UNIT': BEGIN
      Widget_Control, state.textWidgets[4], Get_Value=value
      thm_ui_update_meta,state,'unit',value
    END
    'COORDINATE': BEGIN
      index = event.index
      if index[0] ne -1 then begin
        thm_ui_update_meta,state,'coordinate',index
      endif
    END
    'UP': BEGIN
...
PRO thm_ui_verify_data, gui_id, names, loadedData, windowstorage, historywin, edit=edit,newnames=newnames,success=success,callSequence=callSequence
...
;       arrowBase = Widget_Base(metaFrameBase, /row, /align_center)
  ...
  sectionLabel = Widget_Label(metaLabelBase, value='Meta Data:')  
...
;  rightbmp =filepath('shift_right.bmp', SubDir=['resource', 'bitmaps'])
  ...
  statusBar = Obj_New("THM_UI_MESSAGE_BAR", status_row, Xsize=82, YSize=1)
  ...
  XManager, 'thm_ui_verify_data', tlb
