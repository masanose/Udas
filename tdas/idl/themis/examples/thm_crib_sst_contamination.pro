...
;Now select the bins that have electrical contamination using mouse button 3.
;In this example I selected bins: 32,33,55,56,62
;After the bins have been marked(their bin number should turn white) you can
;exit the interactive mode using mouse button 2.
...
thm_part_getspec, probe='d', trange=['2008-03-12/03:30:00','2008-03-12/04:30:00'], theta=[-45,0], $
  data_type='pseb', angle='phi', suffix='_ex1_t1',mask_remove=.99,method='median'
...
thm_part_getspec, probe=probe, trange=[sdate,edate], theta=[0,45], $
  ...
  method='z_score_mod'  ;using shortened prefix of keyword to enter command
;
;this call will generate spectrograms but remove the sun contamination.
...
thm_part_getspec, probe=probe, trange=[sdate,edate], theta=[0,45], $
  ...
  method='spin_fit'  ;using shortened prefix of keyword to enter command
...
thm_part_getspec, probe=probe, trange=[sdate,edate], theta=[0,45],$
  ...
  limit=1.8 ;this uses the abbreviated version of the limit keyword
