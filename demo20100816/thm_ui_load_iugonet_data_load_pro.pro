...
pro thm_ui_load_iugonet_data_load_pro,$
  ...
  if instrument eq 'gmag_index' then begin
    ...
    if datatype eq 'RISH' then begin
        if site_or_param eq 'pameungpeuk' then begin
           par_names=parameters+'_wind_pam'
           iug_load_mf_pam, datatype ='thermo_wind', parameters=par_names, trange = timeRange
        endif else if site_or_param eq 'pontianak' then begin
           par_names=parameters+'_wind_pon'
           iug_load_mf_pon, datatype ='thermo_wind', parameters=par_names, trange = timeRange
        endif
    endif else if datatype eq 'NIPR' then begin
        par_names='mf_'+site_or_param+'_'+strmid(parameters,0,5)+'_vel'
